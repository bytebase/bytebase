...
func (exec *DatabaseBackupTaskExecutor) RunOnce(ctx context.Context, server *Server, task *api.Task) (terminated bool, result *api.TaskRunResultPayload, err error) {
	...
	dumpSize, backupErr := exec.backupDatabase(ctx, task.Instance, task.Database.Name, backup, server.profile.DataDir)
...
func (exec *DatabaseBackupTaskExecutor) backupDatabase(ctx context.Context, instance *api.Instance, databaseName string, backup *api.Backup, dataDir string) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to open backup path: %s", backup.Path)
	}
	defer f.Close()

	dumpSize, err := driver.Dump(ctx, databaseName, f, false /* schemaOnly */)
	if err != nil {
		return 0, err
	}
