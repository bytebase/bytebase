...
func (s *TaskScheduler) ScheduleIfNeeded(ctx context.Context, task *api.Task) (*api.Task, error) {
	...
	{
		pass, err := s.server.TaskCheckScheduler.passCheck(ctx, s.server, task, api.TaskCheckTimingEarliestAllowedTime)
		if err != nil {
			return nil, err
		}
		if !pass {
	...
	if task.Type == api.TaskDatabaseSchemaUpdate {
		pass, err := s.server.TaskCheckScheduler.passCheck(ctx, s.server, task, api.TaskCheckDatabaseConnect)
		if err != nil {
			return nil, err
		}
		if !pass {
		...
		pass, err = s.server.TaskCheckScheduler.passCheck(ctx, s.server, task, api.TaskCheckInstanceMigrationSchema)
		if err != nil {
			return nil, err
		}
		if !pass {
		...
		if instance.Engine == db.MySQL || instance.Engine == db.TiDB {
			pass, err = s.server.TaskCheckScheduler.passCheck(ctx, s.server, task, api.TaskCheckDatabaseStatementSyntax)
			if err != nil {
				return nil, err
			}
			if !pass {
			...
			pass, err = s.server.TaskCheckScheduler.passCheck(ctx, s.server, task, api.TaskCheckDatabaseStatementCompatibility)
			if err != nil {
				return nil, err
			}
			if !pass {
