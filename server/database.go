...
func (s *Server) composeDatabaseRelationship(ctx context.Context, raw *api.DatabaseRaw) (*api.Database, error) {
	...
	creator, err := s.store.Principal.ComposeByID(ctx, db.CreatorID)
	if err != nil {
		return nil, err
	}
	db.Creator = creator

	updater, err := s.store.Principal.ComposeByID(ctx, db.UpdaterID)
	if err != nil {
		return nil, err
	}
	db.Updater = updater
	...
	for _, anomaly := range db.AnomalyList {
		anomaly.Creator, err = s.store.Principal.ComposeByID(ctx, anomaly.CreatorID)
		if err != nil {
			return nil, err
		}
		anomaly.Updater, err = s.store.Principal.ComposeByID(ctx, anomaly.UpdaterID)
		if err != nil {
			return nil, err
		}
	}
...
func (s *Server) composeTableRelationship(ctx context.Context, raw *api.TableRaw) (*api.Table, error) {
	...
	creator, err := s.store.Principal.ComposeByID(ctx, table.CreatorID)
	if err != nil {
		return nil, err
	}
	table.Creator = creator

	updater, err := s.store.Principal.ComposeByID(ctx, table.UpdaterID)
	if err != nil {
		return nil, err
	}
...
func (s *Server) composeViewRelationship(ctx context.Context, raw *api.ViewRaw) (*api.View, error) {
	...
	creator, err := s.store.Principal.ComposeByID(ctx, view.CreatorID)
	if err != nil {
		return nil, err
	}
	view.Creator = creator

	updater, err := s.store.Principal.ComposeByID(ctx, view.UpdaterID)
	if err != nil {
		return nil, err
	}
...
func (s *Server) composeBackupRelationship(ctx context.Context, raw *api.BackupRaw) (*api.Backup, error) {
	backup := raw.ToBackup()
	creator, err := s.store.Principal.ComposeByID(ctx, backup.CreatorID)
	if err != nil {
		return nil, err
	}
	backup.Creator = creator

	updater, err := s.store.Principal.ComposeByID(ctx, backup.UpdaterID)
	if err != nil {
		return nil, err
	}
