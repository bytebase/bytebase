...
func (s *Server) registerTaskRoutes(g *echo.Group) {
	...
	g.PATCH("/pipeline/:pipelineID/task/:taskID/status", func(c echo.Context) error {
		...
		if issue.AssigneeID == api.SystemBotID {
			currentPrincipal, err := s.Store.Principal.ComposeByID(ctx, currentPrincipalID)
			if err != nil {
...
func (s *Server) composeTaskRelationship(ctx context.Context, raw *api.TaskRaw) (*api.Task, error) {
	...
	creator, err := s.Store.Principal.ComposeByID(ctx, task.CreatorID)
	if err != nil {
		return nil, err
	}
	task.Creator = creator

	updater, err := s.Store.Principal.ComposeByID(ctx, task.UpdaterID)
	if err != nil {
		return nil, err
	}
	...
	for _, taskRun := range task.TaskRunList {
		creator, err := s.Store.Principal.ComposeByID(ctx, taskRun.CreatorID)
		if err != nil {
			return nil, err
		}
		taskRun.Creator = creator

		updater, err := s.Store.Principal.ComposeByID(ctx, taskRun.UpdaterID)
		if err != nil {
			return nil, err
		}
	...
	for _, taskCheckRun := range task.TaskCheckRunList {
		creator, err := s.Store.Principal.ComposeByID(ctx, taskCheckRun.CreatorID)
		if err != nil {
			return nil, err
		}
		taskCheckRun.Creator = creator

		updater, err := s.Store.Principal.ComposeByID(ctx, taskCheckRun.UpdaterID)
		if err != nil {
			return nil, err
		}
...
func (s *Server) composeTaskRelationshipValidateOnly(ctx context.Context, task *api.Task) error {
	...
	task.Creator, err = s.Store.Principal.ComposeByID(ctx, task.CreatorID)
	if err != nil {
		return err
	}

	task.Updater, err = s.Store.Principal.ComposeByID(ctx, task.UpdaterID)
	if err != nil {
		return err
	}

	for _, taskRun := range task.TaskRunList {
		taskRun.Creator, err = s.Store.Principal.ComposeByID(ctx, taskRun.CreatorID)
		if err != nil {
			return err
		}

		taskRun.Updater, err = s.Store.Principal.ComposeByID(ctx, taskRun.UpdaterID)
		if err != nil {
			return err
		}
	}
	...
	for _, taskCheckRun := range task.TaskCheckRunList {
		taskCheckRun.Creator, err = s.Store.Principal.ComposeByID(ctx, taskCheckRun.CreatorID)
		if err != nil {
			return err
		}

		taskCheckRun.Updater, err = s.Store.Principal.ComposeByID(ctx, taskCheckRun.UpdaterID)
		if err != nil {
			return err
		}
	}
