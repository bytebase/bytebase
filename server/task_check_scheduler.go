...
func (s *TaskCheckScheduler) ScheduleCheckIfNeeded(ctx context.Context, task *api.Task, creatorID int, skipIfAlreadyTerminated bool) (*api.Task, error) {
	...
	// In order not to bother user too much, we adopt a somewhat method here:
	{
		...
		isPassedTimingTaskCheck, err := s.passCheck(ctx, s.server, task, api.TaskCheckTimingEarliestAllowedTime)
		if err != nil {
			return nil, err
		}
...
func (s *TaskCheckScheduler) passCheck(ctx context.Context, server *Server, task *api.Task, checkType api.TaskCheckType) (bool, error) {
