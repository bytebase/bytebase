...
func (m *ActivityManager) CreateActivity(ctx context.Context, create *api.ActivityCreate, meta *ActivityMeta) (*api.Activity, error) {
	...
	updater, err := m.s.PrincipalService.Find(ctx, principalFind)
	if err != nil {
		...
		case api.IssueFieldAssignee:
			{
				...
				if update.OldValue != "" {
					...
					oldAssignee, err = m.s.PrincipalService.Find(ctx, principalFind)
					if err != nil {
				...
				if update.NewValue != "" {
					...
					newAssignee, err = m.s.PrincipalService.Find(ctx, principalFind)
					if err != nil {
