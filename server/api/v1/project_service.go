...
// GetProjectPolicy gets a policy in a specific project.
func (s *ProjectService) GetProjectPolicy(ctx context.Context, request *v1pb.GetPolicyRequest) (*v1pb.Policy, error) {
	tokens, err := getNameParentTokens(request.Name, projectNamePrefix, policyNamePrefix)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument, err.Error())
	}
	...
	return convertToPolicy(convertToProject(project).Name, policy), nil
}

// ListProjectPolicies lists policies in a specific project.
func (s *ProjectService) ListProjectPolicies(ctx context.Context, request *v1pb.ListPoliciesRequest) (*v1pb.ListPoliciesResponse, error) {
	...
	prefix := convertToProject(project).Name
