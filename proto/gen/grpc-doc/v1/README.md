...
Known limitations of table/schema-scoped DDL/DML grants: - The scope only gates the write target, not the read sources of a write: e.g. `INSERT INTO granted_table SELECT * FROM other_table` may read `other_table` without a grant on it, so a table-scoped write grant is not an exfiltration boundary. - It must be paired with a database/project-level read grant: a table-scoped grant alone does not satisfy the SQL Editor query method permission (bb.databases.get). - A resource.schema_name-scoped grant only authorizes a write whose schema is unambiguous — qualify the table (schema.table) or select the schema for the SQL Editor session. An unqualified write whose effective schema cannot be determined ahead of execution is denied (qualify it, or use a table-only grant). - Multi-statement batches are authorized at the database level: an earlier statement can rebind the session&#39;s default schema or database mid-batch, so per-table/schema scoping is dropped and a table/schema-scoped grant requires a database-level grant. The database-level check is still per target database — a qualified cross-database write is gated by a grant on its own database, not the request database — but an unqualified write whose session default is rebound mid-batch is evaluated against the database it literally names, so qualify cross-database writes or run them as single statements. - Enforcement is on the statement&#39;s literal write target; writes routed elsewhere by a view/synonym, or into a temporary schema (e.g. pg_temp), are evaluated against what the statement names, not the ultimate base object. - Write-target gating depends on the statement type: - Data/table writes the resolver models to a base table are per-table/schema-scopable: INSERT/UPDATE/DELETE/TRUNCATE/MERGE, LOAD DATA, IMPORT INTO, CREATE TABLE AS / SELECT INTO, and table-level CREATE/DROP/ALTER/RENAME/index. - Qualified non-table object DDL (CREATE/ALTER/DROP of view/procedure/function/trigger/sequence/synonym whose name carries an explicit database/schema) is gated at the DATABASE level by that target database — a qualified cross-database object DDL is checked against its own database, not the request database. Unqualified object DDL keeps the request-database check. - Statement types the resolver does not model fall back to the request-database check. Notably not modeled yet: MySQL CREATE TRIGGER (its AST trigger name is unqualified), MSSQL ALTER of views/routines/triggers, engine-specific bulk-load / deprecated text writes, and COPY ... FROM. - Out of scope: a write&#39;s read sources (INSERT … SELECT, MERGE … USING, CREATE TABLE AS … SELECT) and objects reached indirectly via views/synonyms/function bodies/triggers are NOT gated. Invariant: when a statement explicitly names a different database as its write target AND Bytebase models that statement type, the ACL check uses that target database, not the request database.
