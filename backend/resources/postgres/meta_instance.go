...
func StartMetadataInstance(ctx context.Context, pgDataDir string, port int, mode common.ReleaseMode) (func(), error) {
	...
	if err := initDB(pgDataDir, "bb"); err != nil {
		return nil, err
	}
	serverLog := mode == common.ReleaseModeDev
	if err := start(port, pgDataDir, serverLog); err != nil {
		return nil, err
	}
