...
func TestToStoredVersion(t *testing.T) {
	...
	for _, tc := range tests {
		version := Version{Semantic: tc.useSemanticVersion, Version: tc.version, Suffix: tc.semanticVersionSuffix}
		got, err := version.Marshal()
		if tc.wantErr != "" {
...
func TestFromStoredVersion(t *testing.T) {
	...
	for _, tc := range tests {
		version, err := NewVersion(tc.storedVersion)
		if tc.wantErr != "" {
			require.Contains(t, err.Error(), tc.wantErr)
			continue
		}
		require.NoError(t, err)
		require.Equal(t, tc.wantUseSemanticVersion, version.Semantic)
		require.Equal(t, tc.wantVersion, version.Version)
		require.Equal(t, tc.wantSemanticVersionSuffix, version.Suffix)
	}
}
