...
func TestMigration3_23_1Backport_0_WorkloadIdentityAudiences(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, db.QueryRowContext(ctx,
		`SELECT version FROM instance_change_history ORDER BY id DESC LIMIT 1`).Scan(&latestVersion))
	require.Equal(t, "3.23.1-backport.0", latestVersion)

	// workload_identity.workspace is a foreign key, so the row the fixtures
	// reference has to exist first.
	...
	statement, err := os.ReadFile("../migrator/migration/3.23.1-backport/0000##workload_identity_audiences.sql")
	require.NoError(t, err)
