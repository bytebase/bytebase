package dbauth

import (
	"context"

	"github.com/jackc/pgx/v5"
	"github.com/jackc/pgx/v5/stdlib"
	"github.com/pkg/errors"
)

// Configure applies metadata database authentication settings to pgxConfig.
func Configure(ctx context.Context, pgxConfig *pgx.ConnConfig) ([]stdlib.OptionOpenDB, error) {
	var configures []func() ([]stdlib.OptionOpenDB, error)

	awsConfig, err := awsConfigFromPGXConfig(pgxConfig)
	if err != nil {
		return nil, err
	}
	if awsConfig != nil && awsConfig.enabled {
		configures = append(configures, func() ([]stdlib.OptionOpenDB, error) {
			return configureAWS(ctx, awsConfig)
		})
	}

	gcpConfig, err := gcpConfigFromPGXConfig(pgxConfig)
	if err != nil {
		return nil, err
	}
	if gcpConfig != nil && gcpConfig.enabled {
		configures = append(configures, func() ([]stdlib.OptionOpenDB, error) {
			return configureGCP(ctx, pgxConfig, gcpConfig)
		})
	}

	return configureSingleProvider(configures)
}

func configureSingleProvider(configures []func() ([]stdlib.OptionOpenDB, error)) ([]stdlib.OptionOpenDB, error) {
	switch len(configures) {
	case 0:
		return nil, nil
	case 1:
		return configures[0]()
	default:
		return nil, errors.New("multiple metadata database IAM auth providers are enabled")
	}
}

func configureAWS(ctx context.Context, authConfig *awsConfig) ([]stdlib.OptionOpenDB, error) {
	tokenProvider, err := newAWSMetadataDBTokenProvider(ctx, authConfig.region)
	if err != nil {
		return nil, err
	}
	return awsOpenOptions(authConfig, tokenProvider), nil
}

func configureGCP(ctx context.Context, pgxConfig *pgx.ConnConfig, authConfig *gcpConfig) ([]stdlib.OptionOpenDB, error) {
	dialer, err := newGCPMetadataDBDialer(ctx)
	if err != nil {
		return nil, err
	}
	applyGCPConfig(pgxConfig, authConfig, dialer)
	return nil, nil
}

// IsKeywordValueRuntimeParam reports whether key is a Bytebase metadata DB auth runtime parameter.
func IsKeywordValueRuntimeParam(key string) bool {
	switch key {
	case awsRDSIAMParam, awsRegionParam, gcpCloudSQLIAMParam, gcpCloudSQLInstanceConnectionNameParam:
		return true
	default:
		return false
	}
}
