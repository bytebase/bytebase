...
func listUserImpl(ctx context.Context, txn *sql.Tx, find *FindUserMessage) ([]*UserMessage, error) {
	...
	if v := find.ProjectID; v != nil {
		with = `WITH all_members AS (
			...
			FROM policy
			WHERE ((resource_type = '` + string(base.PolicyResourceTypeProject) + `' AND resource = 'projects/` + *v + `') OR resource_type = '` + string(base.PolicyResourceTypeWorkspace) + `') AND type = '` + string(storepb.PolicyType_IAM) + `'
		),
		project_members AS (
