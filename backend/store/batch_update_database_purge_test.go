...
func TestBatchUpdateDatabasesFailsFastWhenLifecycleBusy(t *testing.T) {
	environmentID, unset := "env-a", ""
	fixture := newProjectDeletionLockOrderFixture(t, `
	...
	_, err = conn.ExecContext(fixture.ctx, "SELECT pg_advisory_lock($1, hashtext($2))", int64(store.AdvisoryLockKeyProjectLifecycle), "default")
	require.NoError(t, err)
	t.Cleanup(func() {
		_, err := conn.ExecContext(fixture.ctx, "SELECT pg_advisory_unlock($1, hashtext($2))", int64(store.AdvisoryLockKeyProjectLifecycle), "default")
		require.NoError(t, err)
	})
	...
	require.NoError(t, fixture.db.QueryRowContext(fixture.ctx,
		"SELECT environment FROM db WHERE instance = 'instance-a' AND name = 'db-a'").Scan(&environment))
	require.Equal(t, environmentID, environment.String)
}
