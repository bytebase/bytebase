...
func (s *Store) CreateTaskRunLogS(ctx context.Context, taskRunUID int, t time.Time, instanceUUID string, e *storepb.TaskRunLog) {
	if err := s.CreateTaskRunLog(ctx, taskRunUID, t, instanceUUID, e); err != nil {
		slog.Error("failed to create task run log", log.BBError(err))
	}
}

func (s *Store) CreateTaskRunLog(ctx context.Context, taskRunUID int, t time.Time, instanceUUID string, e *storepb.TaskRunLog) error {
	query := `
