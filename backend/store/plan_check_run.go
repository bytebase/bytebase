...
func (s *Store) ListPlanCheckRuns(ctx context.Context, find *FindPlanCheckRunMessage) ([]*PlanCheckRunMessage, error) {
	...
	if find.LatestOnly {
		...
		orderBy = "ORDER BY plan_check_run.type, plan_check_run.config->>'instanceUid', plan_check_run.config->>'databaseName', plan_check_run.id"
	} else {
		orderBy = "ORDER BY plan_check_run.id"
	}
	query := fmt.Sprintf(`
SELECT
	%s
