...
// CreateActivityV2 creates the activity.
func (s *Store) CreateActivityV2(ctx context.Context, create *ActivityMessage) (*ActivityMessage, error) {
	...
	activity := ActivityMessage{
		...
		ContainerUID: create.ContainerUID,
		Type:         create.Type,
		Level:        create.Level,
	...
	if err := tx.QueryRowContext(ctx, `
		...
		RETURNING
			id,
			created_ts
	`, create.CreatorID, create.ContainerUID, create.Type, create.Level, create.Comment, payload).Scan(&activity.UID, &activity.CreatedTs); err != nil {
		return nil, err
	}
