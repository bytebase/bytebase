...
func NewServer(ctx context.Context, profile config.Profile) (*Server, error) {
	...
	if profile.UseEmbedDB() {
		stopper, err := postgres.StartMetadataInstance(profile.ResourceDir, s.pgBinDir, profile.DataDir, profile.PgUser, profile.DemoName, profile.DatastorePort, profile.Mode)
		if err != nil {
			return nil, err
		}
