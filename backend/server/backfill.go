...
func backfillBranches(ctx context.Context, stores *store.Store) {
	...
	for _, id := range ids {
		...
		headMetadata, err := schema.ParseToMetadata(branch.Engine, "" /* defaultSchemaName */, string(branch.HeadSchema))
		if err != nil {
			slog.Error("failed to transform head metadata", slog.Int("branchID", id), log.BBError(err))
			continue
		}
		branch.Head.Metadata = headMetadata
		baseMetadata, err := schema.ParseToMetadata(branch.Engine, "" /* defaultSchemaName */, string(branch.BaseSchema))
		if err != nil {
			slog.Error("failed to transform head metadata", slog.Int("branchID", id), log.BBError(err))
			continue
		}
