	...
	for _, createTableStmt := range checker.createdTableStmtList {
		...
		if commentStmt == nil || commentStmt.Comment == "" {
			checker.adviceList = append(checker.adviceList, &storepb.Advice{
				...
				Title:   checker.title,
				Content: fmt.Sprintf("Comment is required for table `%s`", createTableStmt.Name.Name),
			...
			if checker.payload.MaxLength > 0 && len(comment) > checker.payload.MaxLength {
				checker.adviceList = append(checker.adviceList, &storepb.Advice{
					...
					Title:   checker.title,
					Content: fmt.Sprintf("Table `%s` comment is too long. The length of comment should be within %d characters", createTableStmt.Name.Name, checker.payload.MaxLength),
					StartPosition: &storepb.Position{
			...
			if checker.payload.RequiredClassification {
				if classification, _ := common.GetClassificationAndUserComment(comment, checker.classificationConfig); classification == "" {
					checker.adviceList = append(checker.adviceList, &storepb.Advice{
						...
						Title:   checker.title,
						Content: fmt.Sprintf("Table `%s` comment requires classification", createTableStmt.Name.Name),
						StartPosition: &storepb.Position{
							Line: int32(createTableStmt.LastLine()),
						},
					})
