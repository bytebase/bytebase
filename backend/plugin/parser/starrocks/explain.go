...
func init() {
	base.RegisterExplainStatementFunc(storepb.Engine_STARROCKS, explainStatement)
}

// explainStatement implements base.ExplainStatement. StarRocks returns one plan
...
func singleStatement(text string) (ast.Node, error) {
	parsed, err := parseStarRocksSQL(text)
	if err != nil {
		return nil, err
	}
