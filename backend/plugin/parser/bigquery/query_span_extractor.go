...
// querySpanExtractor analyses a single BigQuery (GoogleSQL) statement and
// produces a base.QuerySpan: the set of base tables it accesses, the per-output
// column lineage (expanded against catalog metadata when available), the
// predicate columns it filters on, and its statement classification.
//
// The extractor delegates structural lineage to omni's analysis.GetQuerySpan
// (DialectBigQuery), then applies the bytebase-specific post-processing the
// legacy plugin performed:
//   - default-dataset fill-in for bare table references (BigQuery has no schema
//     layer: a dataset is the database, so omni buckets a 2-part dataset.table
//     as Database=dataset, Table=table, Schema="" — INFORMATION_SCHEMA is the one
//     exception, recorded as Schema), mapped onto base.ColumnResource;
//   - the user/system mixed-table rejection (base.MixUserSystemTablesError);
...
//   - mapping omni's per-output columns + PredicateColumns onto those columns.
//
// This mirrors the proven Trino cutover (#20517) structurally; the deltas are
// the BigQuery name model (no schema layer; a 3-arg constructor with no default
// schema), the INFORMATION_SCHEMA-only system set, and a table-level
// SourceColumns set (BigQuery's legacy span reports accessed tables at the table
// level, not column-expanded — see getQuerySpan).
type querySpanExtractor struct {
	...
	// metaCache memoises database (dataset) metadata lookups within a single span.
	metaCache map[string]*model.DatabaseMetadata
}

// newQuerySpanExtractor creates a new BigQuery query span extractor. Its
// signature matches the legacy 3-arg form: BigQuery has no schema layer, so there
// is no defaultSchema parameter (unlike the Trino template). The
// ignoreCaseSensitive flag is accepted for signature parity but unused — the
// legacy extractor ignored it too (its constructor took `_ bool`) because
// BigQuery object names resolve case-insensitively regardless: tableColumns
// always falls back to a case-folded name match.
...
// getQuerySpan extracts the query span for a single BigQuery statement.
func (q *querySpanExtractor) getQuerySpan(ctx context.Context, statement string) (*base.QuerySpan, error) {
	...
	// legacy behaviour, which parsed via ParseBigQuerySQL and rejected != 1).
	...
	omniSpan, err := analysis.GetQuerySpan(single, analysis.DialectBigQuery)
	if err != nil {
		return nil, err
	}

	// Reject a statement that reads from both user and system (INFORMATION_SCHEMA)
	// tables, matching the legacy isMixedQuery → MixUserSystemTablesError.
	...
	// classifier already promotes this, but recompute from the access-table system
	// flags too so the result is robust if the two ever disagree (the legacy
	// extractor derived allSystems from the access tables, not the classifier).
	...
	span := &base.QuerySpan{
		...
		// PredicateColumns is intentionally left empty: the legacy BigQuery
		// extractor (getQuerySpan) never populated predicate-column lineage — its
		// base.QuerySpan carried only Type, SourceColumns and Results — so the
		// restored goldens expect an empty predicate set. omni DOES collect
		// predicate columns (used by the Spanner side and others), but surfacing
		// them here would diverge from the BigQuery legacy contract the masking
		// layer was calibrated against.
		Results: results,
	}
	if notFound != nil {
...
// base.ColumnResource keys (Column empty), applying the default dataset. The
// table name is kept AS WRITTEN (matching the legacy accessTableListener); the
// project/catalog qualifier is dropped (the legacy listener never recorded the
// project — a 3-part project.dataset.table recorded only the dataset as
// Database).
func (q *querySpanExtractor) toTableResources(span *analysis.QuerySpan) base.SourceColumnSet {
	...
	for _, t := range span.AccessTables {
		out[base.ColumnResource{
			Database: q.resolveDatabase(t.Database),
			Schema:   t.Schema,
			Table:    t.Table,
		}] = true
	}
	...
	// the table-level SourceColumns (with the default dataset filled in, as the
	// legacy listener did for a bare name).
...
// resolveDatabase fills in the default dataset for a bare table reference that
// omitted its dataset qualifier, mirroring BigQuery's session-qualified
// resolution (the legacy code defaulted Database to currentDatabase).
func (q *querySpanExtractor) resolveDatabase(database string) string {
	if database == "" {
...
func (q *querySpanExtractor) expandTablesToColumns(span *analysis.QuerySpan) ([]base.ColumnResource, base.SourceColumnSet, *base.ResourceNotFoundError) {
	...
	for _, t := range span.AccessTables {
		db := q.resolveDatabase(t.Database)
		if t.IsSystem {
			// INFORMATION_SCHEMA references are system pseudo-objects, not
			// Bytebase-tracked tables; their metadata is never masked. Resolving them
			// via the metadata getter would yield a ResourceNotFoundError that
			// sql_service turns into a hard "failed to mask" rejection of an otherwise
			// successful result, so keep the table-level resource and skip the lookup.
			// Such queries are already classified SelectInfoSchema (and a mixed
			// user/system query is rejected earlier). Detection uses omni's per-dialect
			// IsSystem flag (Schema == INFORMATION_SCHEMA), not a query-text scan.
			add(base.ColumnResource{Database: db, Schema: t.Schema, Table: t.Table})
			continue
		}
		canonicalTable, columns, err := q.tableColumns(db, t.Table)
		if err != nil {
...
// order, line up.
//
// Best-effort boundaries (each matching the merged omni analysis peers and the
// Trino cutover; tracked as follow-ups rather than blocking the cutover):
//   - omni records a star ("*"/"t.*") by name and does not expose the resolved
//     output relation, so SELECT * over a CTE or derived table that projects a
//     subset/reordering expands to the underlying base table's columns;
//   - omni does not model SELECT * EXCEPT/REPLACE, so those modifiers are not
//     applied (the star expands to all columns; an EXCEPT-ed column is not
//     removed — over-including, which over-masks rather than under-masks — and a
//     REPLACE expression is not substituted);
//   - a column reference qualified by a derived-relation alias (e.g. `result.ID`
//     where `result` is a subquery alias, not a base table) finds no match in the
//     metadata-expanded base columns and yields empty SourceColumns for that
//     output position. This is the one boundary that can UNDER-mask a named
//     output; in the golden corpus the same physical column is also surfaced by a
//     sibling `*` (so the underlying column stays masked), but a query projecting
//     only such a qualified derived reference would leave that position unmasked.
//     A correct fix needs omni to expose a derived relation's resolved output
//     projection; tracked as a follow-up (same boundary the Trino/doris/snowflake
//     cutovers accepted), not blocking this cutover;
//   - an expression select item carries omni's best-effort name (the leading
//     column name, or "" for a multi-column/opaque expression) rather than the
//     legacy uppercased rendering.
...
func (q *querySpanExtractor) expandBaseTableColumns(rel analysis.ColumnRef, plain bool, except []string) []base.QuerySpanResult {
	db := q.resolveDatabase(rel.Database)
	canonicalTable, columns, err := q.tableColumns(db, rel.Table)
...
func (q *querySpanExtractor) buildAliasMap(span *analysis.QuerySpan) map[string][]string {
	...
	for _, t := range span.AccessTables {
		...
		table := t.Table
		if !t.IsSystem {
			if canonical, _, err := q.tableColumns(q.resolveDatabase(t.Database), t.Table); err == nil && canonical != "" {
				table = canonical
			}
		}
...
// the given table or view in the given dataset, honouring the case-sensitivity
// flag for object lookup. BigQuery has no schema layer, so the lookup uses the
// dataset's single unnamed schema ("") — matching the legacy findTableSchema.
func (q *querySpanExtractor) tableColumns(db, table string) (string, []string, error) {
	metadata, err := q.getDatabaseMetadata(db)
	if err != nil {
		return "", nil, err
	}

	// BigQuery datasets have a single unnamed schema.
	schemaMeta := metadata.GetSchemaMetadata("")
	if schemaMeta == nil {
...
// getDatabaseMetadata fetches (and caches) metadata for the given dataset.
