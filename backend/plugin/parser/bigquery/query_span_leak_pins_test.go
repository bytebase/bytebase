...
func leakPinSpan(t *testing.T, statement string, tables []*storepb.TableMetadata) *base.QuerySpan {
	t.Helper()
	meta := &storepb.DatabaseSchemaMetadata{Name: "ds1", Schemas: []*storepb.SchemaMetadata{{Name: "", Tables: tables}}}
	...
	}, base.Statement{Text: statement}, "ds1", "dbo", false)
...
func TestLeakPin_LowercaseUsingCoalesce(t *testing.T) {
	span := leakPinSpan(t, "SELECT * FROM a JOIN b USING (k);", []*storepb.TableMetadata{
		{Name: "a", Columns: []*storepb.ColumnMetadata{{Name: "k"}, {Name: "x"}}},
		{Name: "b", Columns: []*storepb.ColumnMetadata{{Name: "k"}, {Name: "y"}}},
	})
	require.Len(t, span.Results, 3, "USING key must be coalesced: real output is [k, x, y]")
	require.Equal(t, []string{"a.k", "b.k"}, sourcesOf(span.Results[0]), "coalesced key reads both sides")
	require.Equal(t, []string{"a.x"}, sourcesOf(span.Results[1]))
	require.Equal(t, []string{"b.y"}, sourcesOf(span.Results[2]))
}
...
func TestLeakPin_UnnestLineage(t *testing.T) {
	span := leakPinSpan(t, "SELECT elem FROM victim, UNNEST(victim.secret_tokens) AS elem", []*storepb.TableMetadata{
...
func TestLeakPin_UnnestWithOffsetNoLineage(t *testing.T) {
	span := leakPinSpan(t, "SELECT elem, pos FROM victim, UNNEST(victim.secret_tokens) AS elem WITH OFFSET AS pos", []*storepb.TableMetadata{
		{Name: "victim", Columns: []*storepb.ColumnMetadata{{Name: "id"}, {Name: "secret_tokens"}}},
	})
	require.Len(t, span.Results, 2)
	require.Equal(t, []string{"victim.secret_tokens"}, sourcesOf(span.Results[0]))
	require.Empty(t, sourcesOf(span.Results[1]), "OFFSET is positional, carries no data lineage")
}
...
func TestLeakPin_ByNameMerge(t *testing.T) {
	span := leakPinSpan(t, "SELECT id, label FROM lt UNION ALL BY NAME SELECT b_secret AS label, id FROM rt", []*storepb.TableMetadata{
...
func TestLeakPin_ByNameStarArmReversedOrder(t *testing.T) {
	span := leakPinSpan(t, "SELECT * FROM pub UNION ALL BY NAME SELECT secret AS label, id FROM priv", []*storepb.TableMetadata{
		{Name: "pub", Columns: []*storepb.ColumnMetadata{{Name: "id"}, {Name: "label"}}},
		{Name: "priv", Columns: []*storepb.ColumnMetadata{{Name: "id"}, {Name: "secret"}}},
	})
	require.Len(t, span.Results, 2)
	require.Equal(t, []string{"priv.id", "pub.id"}, sourcesOf(span.Results[0]),
		"id merges by NAME despite the right arm listing it second")
	require.Equal(t, []string{"priv.secret", "pub.label"}, sourcesOf(span.Results[1]),
...
func TestLeakPin_ByNameOnMatchColumns(t *testing.T) {
	span := leakPinSpan(t, "SELECT * FROM pub UNION ALL BY NAME ON (label) SELECT id, secret AS label FROM priv", []*storepb.TableMetadata{
		{Name: "pub", Columns: []*storepb.ColumnMetadata{{Name: "id"}, {Name: "label"}}},
		{Name: "priv", Columns: []*storepb.ColumnMetadata{{Name: "id"}, {Name: "secret"}}},
	})
	require.Len(t, span.Results, 1, "ON (label) restricts the output to exactly the listed column")
	require.Equal(t, "label", span.Results[0].Name)
	require.Equal(t, []string{"priv.secret", "pub.label"}, sourcesOf(span.Results[0]),
