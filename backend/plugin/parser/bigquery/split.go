...
// GoogleSQL splitter, then converts each Segment into a base.Statement with the
// position fields bytebase expects. Positions are computed in a single O(n) pass
// via the shared base.ByteOffsetPositionMapper (segment offsets are queried in
// increasing order). Mirrors the Trino cutover (#20517).
