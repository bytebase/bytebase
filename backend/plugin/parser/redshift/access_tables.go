...
func (a *accessTableExtractor) EnterQualified_name(qn *parser.Qualified_nameContext) {
	if a.err != nil {
		return
	}
	...
	qnLength := 1
	directions = append(directions, normalizeRedshiftColid(qn.Colid()))
	if qn.Indirection() != nil {
		allIndirectionElements := qn.Indirection().AllIndirection_el()
		for _, el := range allIndirectionElements {
			if el.Attr_name() != nil {
				directions = append(directions, normalizeRedshiftCollabel(el.Attr_name().Collabel()))
				qnLength++
				break
			}
			continue
