...
func (extractor *fieldExtractor) extractSnowsqlSensitiveFieldsQueryStatement(ctx parser.IQuery_statementContext) ([]base.FieldInfo, error) {
	...
	for i, setOperator := range allSetOperators {
		...
		for i := range right {
			finalLevel := result[i].MaskingAttrbutes
			if cmp.Less[storepb.MaskingLevel](finalLevel, right[i].MaskingAttrbutes) {
				finalLevel = right[i].MaskingAttrbutes
			}
			result[i].MaskingAttrbutes = finalLevel
		}
	}
	return result, nil
}
...
func (extractor *fieldExtractor) evalSnowSQLExprMaskingLevel(ctx antlr.RuleContext) (string, storepb.MaskingLevel, error) {
	switch ctx := ctx.(type) {
...
func (extractor *fieldExtractor) extractSnowsqlSensitiveFieldsJoinClause(ctx parser.IJoin_clauseContext, left []base.FieldInfo) ([]base.FieldInfo, error) {
	...
	if ctx.NATURAL() != nil {
		...
		for _, field := range right {
			if idx, ok := leftMap[field.Name]; !ok {
			...
			} else if cmp.Less[storepb.MaskingLevel](left[idx].MaskingAttrbutes, field.MaskingAttrbutes) {
				...
				result[idx].MaskingAttrbutes = field.MaskingAttrbutes
			}
		}
		return result, nil
	}
		...
		if v := ctx.Pivot_unpivot(); v.PIVOT() != nil {
			...
			for _, literal := range v.AllLiteral() {
				result = append(result, base.FieldInfo{
					...
					MaskingAttrbutes: pivotColumnInOriginalResult.MaskingAttrbutes,
				})
			}
