...
func (extractor *fieldExtractor) extractSnowsqlSensitiveFieldsQueryStatement(ctx parser.IQuery_statementContext) ([]base.FieldInfo, error) {
	...
	for i, setOperator := range allSetOperators {
		...
		for i := range right {
			finalLevel := result[i].MaskingAttributes
			if cmp.Less[storepb.MaskingLevel](finalLevel, right[i].MaskingAttributes) {
				finalLevel = right[i].MaskingAttributes
			}
			result[i].MaskingAttributes = finalLevel
		}
	}
	return result, nil
}
...
func (extractor *fieldExtractor) extractSnowsqlSensitiveFieldsJoinClause(ctx parser.IJoin_clauseContext, left []base.FieldInfo) ([]base.FieldInfo, error) {
	...
	if ctx.NATURAL() != nil {
		...
		for _, field := range right {
			if idx, ok := leftMap[field.Name]; !ok {
			...
			} else if cmp.Less[storepb.MaskingLevel](left[idx].MaskingAttributes, field.MaskingAttributes) {
				...
				result[idx].MaskingAttributes = field.MaskingAttributes
			}
		}
		return result, nil
	}
