...
func TestProcedureDropMigration(t *testing.T) {
	tests := []struct {
		name              string
		previousSDL       string
		currentSDL        string
	...
	}{
		{
			...
			`,
			currentSDL:        ``,
			expectedMigration: `DROP PROCEDURE IF EXISTS "public"."update_user_name"(user_id integer, new_name character varying);`,
		},
		{
			...
			`,
			currentSDL:        ``,
			expectedMigration: `DROP FUNCTION IF EXISTS "public"."calculate_tax"(amount numeric);`,
		},
	}
...
func TestDropProcedureShouldNotAffectFunctionComment(t *testing.T) {
	// This test reproduces the issue shown in the screenshot:
	// When dropping a procedure, the system should not generate
	// COMMENT ON FUNCTION statements that remove comments from other functions
	tests := []struct {
		name        string
		previousSDL string
		currentSDL  string
		...
		{
			name: "Drop procedure with quoted identifiers should not affect function comment",
			// This tests the case where COMMENT statements use quoted identifiers (as generated by the database)
			previousSDL: `
				CREATE OR REPLACE FUNCTION test_function()
				RETURNS void
				LANGUAGE plpgsql
		...
		{
			name: "Drop procedure with wrongly labeled FUNCTION comment",
			// This simulates a bug where procedure comment was mistakenly written as COMMENT ON FUNCTION
			previousSDL: `
				CREATE OR REPLACE FUNCTION test_function()
				RETURNS void
				LANGUAGE plpgsql
				...
				COMMENT ON FUNCTION "public"."new_procedure()" IS 'Wrong: should be PROCEDURE not FUNCTION';
			`,
			currentSDL: `
				CREATE OR REPLACE FUNCTION test_function()
				RETURNS void
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Contains(t, migrationSQL, "DROP PROCEDURE IF EXISTS")
			assert.Contains(t, migrationSQL, "new_procedure")

			// Should NOT touch the function's comment
			// The migration should NOT contain any COMMENT statement about test_function
			if strings.Contains(migrationSQL, "test_function") && strings.Contains(migrationSQL, "COMMENT") {
				// If there's a COMMENT statement about test_function, it should not be setting it to NULL
				assert.NotContains(t, migrationSQL, "COMMENT ON FUNCTION \"public\".test_function() IS NULL",
					"Migration should not remove test_function's comment")
				assert.NotContains(t, migrationSQL, "COMMENT ON FUNCTION \"public\".\"test_function()\" IS NULL",
					"Migration should not remove test_function's comment")
				t.Errorf("Found unexpected COMMENT statement about test_function in migration: %s", migrationSQL)
			}
		})
	}
}
