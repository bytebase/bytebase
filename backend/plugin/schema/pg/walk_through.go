...
func (l *pgCatalogListener) EnterIndexstmt(ctx *parser.IndexstmtContext) {
	...
	l.currentLine = ctx.GetStart().GetLine()
	// Extract relation (table/materialized view) name
	...
	if table == nil && matView == nil {
		l.advice = &storepb.Advice{
			...
			Title:         fmt.Sprintf("Table `%s` does not exist", relationName),
			Content:       fmt.Sprintf("Table `%s` does not exist", relationName),
	...
	if len(columnList) == 0 {
		l.advice = &storepb.Advice{
			...
			Title:         fmt.Sprintf("Index %q in table %q has empty key", indexName, relationName),
			Content:       fmt.Sprintf("Index %q in table %q has empty key", indexName, relationName),
			StartPosition: &storepb.Position{Line: int32(l.currentLine)},
		}
		return
	}
	...
	if indexName == "" {
		indexName = generateIndexName(relationName, columnList, isUnique)
	}
	...
	// Create index - handle table and materialized view separately
	index := &storepb.IndexMetadata{
		Name:        indexName,
		Expressions: columnList,
		Type:        indexType,
	...
	if table != nil {
		...
		if table.GetIndex(indexName) != nil {
			...
			if wasAutoGenerated {
				indexName = generateUniqueIndexName(schema, relationName, columnList, isUnique)
			...
			if colName != "expr" {
				if table.GetColumn(colName) == nil {
					l.advice = &storepb.Advice{
						Status:        storepb.Advice_ERROR,
						Code:          code.ColumnNotExists.Int32(),
						Title:         fmt.Sprintf("Column `%s` does not exist in table `%s`", colName, relationName),
						Content:       fmt.Sprintf("Column `%s` does not exist in table `%s`", colName, relationName),
						StartPosition: &storepb.Position{Line: int32(l.currentLine)},
					}
					return
				}
		...
		// Check if index name already exists
		for _, existingIndex := range matView.Indexes {
		...
		// For materialized views, we don't have column metadata to validate
		// Just add the index to the materialized view
		matView.Indexes = append(matView.Indexes, index)
	}
}
...
func (l *pgCatalogListener) EnterDropstmt(ctx *parser.DropstmtContext) {
	...
	if ctx.Object_type_any_name() != nil {
		...
		} else if objType.MATERIALIZED() != nil && objType.VIEW() != nil {
			// DROP MATERIALIZED VIEW
			if ctx.Any_name_list() != nil {
				for _, anyName := range ctx.Any_name_list().AllAny_name() {
					if err := l.dropMaterializedView(anyName, ifExists); err != nil {
						l.advice = err
						return
					}
				}
...
func (l *pgCatalogListener) dropMaterializedView(anyName parser.IAny_nameContext, ifExists bool) *storepb.Advice {
	parts := pgparser.NormalizePostgreSQLAnyName(anyName)
	if len(parts) == 0 {
		return nil
	}
...
func (l *pgCatalogListener) dropIndex(anyName parser.IAny_nameContext, ifExists bool) *storepb.Advice {
	...
	// Get the owner name (table or materialized view)
	ownerName := index.GetTableProto().Name
	// Try dropping from table first
	table := schema.GetTable(ownerName)
	if table != nil {
		if dropErr := table.DropIndex(indexName); dropErr != nil {
			return &storepb.Advice{
				Status:        storepb.Advice_ERROR,
				Code:          code.IndexNotExists.Int32(),
				Title:         dropErr.Error(),
	...
	// Try dropping from materialized view
	if dropErr := schema.DropMaterializedViewIndex(ownerName, indexName); dropErr != nil {
		return &storepb.Advice{
			Status:        storepb.Advice_ERROR,
			Code:          code.IndexNotExists.Int32(),
			Title:         dropErr.Error(),
...
// ========================================
// CREATE MATERIALIZED VIEW handling
// ========================================
// EnterCreatematviewstmt handles CREATE MATERIALIZED VIEW statements.
func (l *pgCatalogListener) EnterCreatematviewstmt(ctx *parser.CreatematviewstmtContext) {
	if !isTopLevel(ctx.GetParent()) || l.advice != nil {
		return
	}
	l.currentLine = ctx.GetStart().GetLine()
	...
	viewName := extractTableName(ctx.Create_mv_target().Qualified_name())
	schemaName := extractSchemaName(ctx.Create_mv_target().Qualified_name())
	databaseName := extractDatabaseName(ctx.Create_mv_target().Qualified_name())
	// Check if accessing other database
	if databaseName != "" && l.databaseState.DatabaseName() != databaseName {
		l.advice = &storepb.Advice{
			Status:        storepb.Advice_WARNING,
	...
	if schema.GetMaterializedView(viewName) != nil {
		if ifNotExists {
			return
		}
		l.advice = &storepb.Advice{
			Status:        storepb.Advice_ERROR,
			Code:          code.TableExists.Int32(),
			Title:         fmt.Sprintf("Materialized view %q already exists in schema %q", viewName, schema.GetProto().Name),
			Content:       fmt.Sprintf("Materialized view %q already exists in schema %q", viewName, schema.GetProto().Name),
			StartPosition: &storepb.Position{Line: int32(l.currentLine)},
		}
		return
	}
	...
	_, createErr := schema.CreateMaterializedView(viewName, definition)
	if createErr != nil {
		l.advice = &storepb.Advice{
			Status:        storepb.Advice_ERROR,
			Code:          code.TableExists.Int32(),
