...
CREATE SPATIAL INDEX [idx_locations_position] ON [geo].[locations] ([position]);
CREATE SPATIAL INDEX [idx_locations_region] ON [geo].[locations] ([region]);
GO
`,
			migrationDDL: `
-- Add new spatial column
ALTER TABLE [geo].[locations] ADD [boundary] GEOMETRY;
GO

-- Create new spatial index
CREATE SPATIAL INDEX [idx_locations_boundary] ON [geo].[locations] ([boundary]);
GO

-- Drop an existing spatial index
DROP INDEX [idx_locations_position] ON [geo].[locations];
GO
`,
			description: "Test spatial index operations - create and drop spatial indexes",
		},
		{
			name: "comprehensive_spatial_indexes",
			initialSchema: `
CREATE SCHEMA [spatial_test];
GO

-- Table with GEOMETRY columns
CREATE TABLE [spatial_test].[cities] (
    [city_id] INT IDENTITY(1,1) PRIMARY KEY,
    [city_name] NVARCHAR(100) NOT NULL,
    [city_center] GEOMETRY NOT NULL,
    [city_boundary] GEOMETRY,
    [tourist_spots] GEOMETRY
);
GO

-- Table with GEOGRAPHY columns
CREATE TABLE [spatial_test].[countries] (
    [country_id] INT IDENTITY(1,1) PRIMARY KEY,
    [country_name] NVARCHAR(100) NOT NULL,
    [capital_location] GEOGRAPHY NOT NULL,
    [border] GEOGRAPHY,
    [coastline] GEOGRAPHY
);
GO

-- Table with mixed spatial types
CREATE TABLE [spatial_test].[landmarks] (
    [landmark_id] INT IDENTITY(1,1) PRIMARY KEY,
    [landmark_name] NVARCHAR(200) NOT NULL,
    [exact_location] GEOMETRY NOT NULL,
    [geographic_location] GEOGRAPHY NOT NULL,
    [shape] GEOMETRY,
    [elevation] DECIMAL(10,2)
);
GO

-- Create various spatial indexes
CREATE SPATIAL INDEX [idx_cities_center] ON [spatial_test].[cities] ([city_center]);
CREATE SPATIAL INDEX [idx_cities_boundary] ON [spatial_test].[cities] ([city_boundary]);
CREATE SPATIAL INDEX [idx_countries_capital] ON [spatial_test].[countries] ([capital_location]);
CREATE SPATIAL INDEX [idx_landmarks_exact] ON [spatial_test].[landmarks] ([exact_location]);
CREATE SPATIAL INDEX [idx_landmarks_geo] ON [spatial_test].[landmarks] ([geographic_location]);
GO

-- Also create some regular indexes for comparison
CREATE INDEX [idx_cities_name] ON [spatial_test].[cities] ([city_name]);
CREATE UNIQUE INDEX [idx_countries_name] ON [spatial_test].[countries] ([country_name]);
CREATE CLUSTERED INDEX [idx_landmarks_name] ON [spatial_test].[landmarks] ([landmark_name]);
GO
`,
			migrationDDL: `
-- Add new spatial columns
ALTER TABLE [spatial_test].[cities] ADD [metropolitan_area] GEOMETRY;
ALTER TABLE [spatial_test].[countries] ADD [territorial_waters] GEOGRAPHY;
GO

-- Create new spatial indexes
CREATE SPATIAL INDEX [idx_cities_metro] ON [spatial_test].[cities] ([metropolitan_area]);
CREATE SPATIAL INDEX [idx_countries_waters] ON [spatial_test].[countries] ([territorial_waters]);
CREATE SPATIAL INDEX [idx_cities_spots] ON [spatial_test].[cities] ([tourist_spots]);
CREATE SPATIAL INDEX [idx_countries_border] ON [spatial_test].[countries] ([border]);
CREATE SPATIAL INDEX [idx_landmarks_shape] ON [spatial_test].[landmarks] ([shape]);
GO

-- Drop some existing spatial indexes
DROP INDEX [idx_cities_boundary] ON [spatial_test].[cities];
DROP INDEX [idx_landmarks_geo] ON [spatial_test].[landmarks];
GO

-- Recreate with different names
CREATE SPATIAL INDEX [idx_cities_boundary_v2] ON [spatial_test].[cities] ([city_boundary]);
CREATE SPATIAL INDEX [idx_landmarks_geographic] ON [spatial_test].[landmarks] ([geographic_location]);
GO

-- Add regular index to ensure they're not affected
CREATE INDEX [idx_landmarks_elevation] ON [spatial_test].[landmarks] ([elevation]);
GO
`,
			description: "Comprehensive spatial index test with GEOMETRY and GEOGRAPHY types, multiple indexes per table",
		},
		{
			name: "spatial_index_edge_cases",
			initialSchema: `
CREATE SCHEMA [spatial_edge];
GO

-- Table with nullable spatial columns
CREATE TABLE [spatial_edge].[nullable_spatial] (
    [id] INT IDENTITY(1,1) PRIMARY KEY,
    [required_geo] GEOMETRY NOT NULL,
    [optional_geo] GEOMETRY NULL,
    [required_geog] GEOGRAPHY NOT NULL,
    [optional_geog] GEOGRAPHY NULL
);
GO

-- Create spatial indexes on both nullable and non-nullable columns
CREATE SPATIAL INDEX [idx_required_geo] ON [spatial_edge].[nullable_spatial] ([required_geo]);
CREATE SPATIAL INDEX [idx_optional_geo] ON [spatial_edge].[nullable_spatial] ([optional_geo]);
CREATE SPATIAL INDEX [idx_required_geog] ON [spatial_edge].[nullable_spatial] ([required_geog]);
CREATE SPATIAL INDEX [idx_optional_geog] ON [spatial_edge].[nullable_spatial] ([optional_geog]);
GO

-- Table with computed columns referencing spatial data
CREATE TABLE [spatial_edge].[computed_spatial] (
    [id] INT IDENTITY(1,1) PRIMARY KEY,
    [location] GEOMETRY NOT NULL,
    [location_x] AS [location].STX PERSISTED,
    [location_y] AS [location].STY PERSISTED,
    [area] AS [location].STArea()
);
GO

CREATE SPATIAL INDEX [idx_computed_location] ON [spatial_edge].[computed_spatial] ([location]);
GO

-- Table with spatial column as part of composite primary key (edge case)
CREATE TABLE [spatial_edge].[routes] (
    [route_id] INT NOT NULL,
    [segment_id] INT NOT NULL,
    [path] GEOMETRY NOT NULL,
    [distance] DECIMAL(10,2),
    CONSTRAINT [PK_routes] PRIMARY KEY ([route_id], [segment_id])
);
GO

CREATE SPATIAL INDEX [idx_routes_path] ON [spatial_edge].[routes] ([path]);
GO
`,
			migrationDDL: `
-- Test renaming spatial indexes (drop and recreate)
DROP INDEX [idx_optional_geo] ON [spatial_edge].[nullable_spatial];
CREATE SPATIAL INDEX [idx_nullable_geometry] ON [spatial_edge].[nullable_spatial] ([optional_geo]);
GO

-- Add spatial column to table with computed columns
ALTER TABLE [spatial_edge].[computed_spatial] ADD [boundary] GEOMETRY;
GO

CREATE SPATIAL INDEX [idx_computed_boundary] ON [spatial_edge].[computed_spatial] ([boundary]);
GO

-- Add more spatial indexes to routes table
ALTER TABLE [spatial_edge].[routes] ADD [waypoints] GEOGRAPHY;
GO

CREATE SPATIAL INDEX [idx_routes_waypoints] ON [spatial_edge].[routes] ([waypoints]);
GO

-- Test dropping spatial index on nullable column
DROP INDEX [idx_optional_geog] ON [spatial_edge].[nullable_spatial];
GO
`,
			description: "Edge cases for spatial indexes: nullable columns, computed columns, composite keys",
		},
	}
