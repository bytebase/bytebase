...
// TestGetDatabaseMetadata pins what the parser extracts from each schema
// text. The goldens describe this package's own output, not Oracle's catalog:
...
func TestGetDatabaseMetadataObjectKinds(t *testing.T) {
	tests := []struct {
		name   string
		ddl    string
...
`,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			metadata, err := GetDatabaseMetadata(tc.ddl)
			require.NoError(t, err)
...
func TestGetDatabaseMetadataParsesSlashTerminatedPLSQLScript(t *testing.T) {
