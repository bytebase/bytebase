...
func (s *IssueService) SearchIssues(ctx context.Context, req *connect.Request[v1pb.SearchIssuesRequest]) (*connect.Response[v1pb.SearchIssuesResponse], error) {
	...
	if projectID == "-" {
		...
		if !ok {
			return nil, common.PermissionDeniedError(ctx, errors.Errorf("user does not have permission %q in %q", permission.IssuesGet, req.Msg.Parent))
		}
...
func (s *IssueService) buildIssueMessage(ctx context.Context, project *store.ProjectMessage, userEmail string, request *v1pb.CreateIssueRequest, labels []string) (*store.IssueMessage, error) {
	...
	case v1pb.Issue_ROLE_GRANT:
		...
		if request.Issue.RoleGrant.GetRole() != fmt.Sprintf("roles/%s", store.ProjectOwnerRole) {
			workspaceProfileSetting, err := s.store.GetWorkspaceProfileSetting(ctx, common.GetWorkspaceIDFromContext(ctx))
			if err != nil {
...
func (s *IssueService) UpdateIssue(ctx context.Context, req *connect.Request[v1pb.UpdateIssueRequest]) (*connect.Response[v1pb.Issue], error) {
	...
	if err != nil {
		if connect.CodeOf(err) == connect.CodeNotFound && req.Msg.AllowMissing {
			...
			if !ok {
				return nil, common.PermissionDeniedError(ctx, errors.Errorf("user does not have permission %q", permission.IssuesCreate))
			}
...
func (s *IssueService) UpdateIssueComment(ctx context.Context, req *connect.Request[v1pb.UpdateIssueCommentRequest]) (*connect.Response[v1pb.IssueComment], error) {
	...
	if issueComment == nil {
		if req.Msg.AllowMissing {
			...
			if !ok {
				return nil, common.PermissionDeniedError(ctx, errors.Errorf("user does not have permission %q", permission.IssueCommentsCreate))
			}
