...
func queryRetry(
	...
	if err != nil {
		// If splitting fails, fall back to executing as a single statement
		slog.Debug("failed to split multi SQL, executing as single statement", log.BBError(err))
		singleSQLs = []parserbase.SingleSQL{{Text: statement, BaseLine: 0}}
	}
