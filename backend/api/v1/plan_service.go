...
func (s *PlanService) UpdatePlan(ctx context.Context, request *connect.Request[v1pb.UpdatePlanRequest]) (*connect.Response[v1pb.Plan], error) {
	...
	if oldPlan == nil {
		if req.AllowMissing {
			...
			if !ok {
				return nil, common.PermissionDeniedError(ctx, errors.Errorf("user does not have permission %q", permission.PlansCreate))
			}
	...
	if !ok {
		return nil, common.PermissionDeniedError(ctx, errors.Errorf("permission denied to update plan"))
	}
