...
-- QUERY_DATA policy to the WORKSPACE_PROFILE setting (as sqlTimeout)
...
SELECT
    'WORKSPACE_PROFILE',
    jsonb_build_object('sqlTimeout', query_timeout.timeout)
FROM query_timeout
ON CONFLICT (name)
DO UPDATE SET
    value = CASE
        WHEN setting.value IS NULL OR setting.value = '{}'::jsonb THEN
            jsonb_build_object('sqlTimeout', (SELECT timeout FROM query_timeout))
        ELSE
            setting.value || jsonb_build_object('sqlTimeout', (SELECT timeout FROM query_timeout))
    END;
