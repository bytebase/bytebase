...
LEFT JOIN LATERAL (
	SELECT CASE a.type
		...
		WHEN 'bb.pipeline.task.general.earliest-allowed-time.update' THEN
			jsonb_build_object('taskUpdate',jsonb_build_object('fromEarliestAllowedTime',to_timestamp(COALESCE(CAST(payload->>'oldEarliestAllowedTs' AS INTEGER), 0)) AT TIME ZONE 'UTC','toEarliestAllowedTime',to_timestamp(COALESCE(CAST(payload->>'newEarliestAllowedTs' AS INTEGER), 0)) AT TIME ZONE 'UTC','tasks', (SELECT jsonb_build_array('projects/'||ai.project_rid||'/rollouts/'||task.pipeline_id||'/stages/'||task.stage_id||'/tasks/'||task.id) FROM task WHERE task.id::text=a.payload->>'taskId')))
