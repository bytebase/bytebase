...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.17.6"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.17/0006##per_project_auto_increment.sql", files[len(files)-1].path)
}
