...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.22.11"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.22/0011##sample_project_instance.sql", files[len(files)-1].path)
}
