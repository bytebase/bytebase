...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.17.8"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.17/0008##cross_workspace_uuid_pk.sql", files[len(files)-1].path)
}
