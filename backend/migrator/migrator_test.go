...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.22.6"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.22/0006##rename_worksheet_to_saved_query.sql", files[len(files)-1].path)
}
