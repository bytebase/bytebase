...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.17.12"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.17/0012##classification_level_to_int.sql", files[len(files)-1].path)
}
