...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.20.0"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.20/0000##migrate_announcement_level_to_theme.sql", files[len(files)-1].path)
}
