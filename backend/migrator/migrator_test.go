...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.15.2"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.15/0002##add_category_to_release.sql", files[len(files)-1].path)
}
