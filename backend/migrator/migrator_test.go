...
// TestMigration3_23_4_WorkloadIdentityAudiences covers the 3.23.4 backfill.
//
// Bytebase shipped two audience conventions and stored neither: the in-product
// GitOps generator requests "bytebase", the docs request the provider default.
// The migration records "bytebase" for every repairable row, because that is
// what our own generator asks for whatever the provider, and appends the
// provider default only where the row's issuer and subject make it derivable.
//
// A row whose subject is unusable still receives the literal. It then fails on
// the subject rather than on the audience, which is the accurate reason.
func TestMigration3_23_4_WorkloadIdentityAudiences(t *testing.T) {
	...
	statement, err := migrationFS.ReadFile("migration/3.23/0004##workload_identity_audiences.sql")
	require.NoError(t, err)
