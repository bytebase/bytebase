...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.17.11"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.17/0011##idp_nullable_workspace.sql", files[len(files)-1].path)
}
