...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.17.0"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.17/0000##rename_grant_request_to_role_grant.sql", files[len(files)-1].path)
}
