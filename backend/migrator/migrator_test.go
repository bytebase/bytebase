...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.16.2"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.16/0002##drop_unused_id_columns.sql", files[len(files)-1].path)
}
