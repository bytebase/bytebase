...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.14.15"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.14/0015##task_run_available_status.sql", files[len(files)-1].path)
}
