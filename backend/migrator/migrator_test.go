...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.16.5"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.16/0005##migrate_jsonb_ids_to_resource_id.sql", files[len(files)-1].path)
}
