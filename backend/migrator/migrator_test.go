...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.14.19"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.14/0019##merge_database_sdl_to_migrate.sql", files[len(files)-1].path)
}
