...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.20.3"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.20/0003##migrate_color_string_to_message.sql", files[len(files)-1].path)
}
