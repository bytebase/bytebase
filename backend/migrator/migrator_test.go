...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.16.6"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.16/0006##add_project_to_issue_chain.sql", files[len(files)-1].path)
}
