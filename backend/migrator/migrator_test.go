...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.19.2"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.19/0002##migrate_announcement_level_to_theme.sql", files[len(files)-1].path)
}
