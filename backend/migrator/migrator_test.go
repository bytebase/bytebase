...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.16.1"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.16/0001##fix_workspace_policy_resource.sql", files[len(files)-1].path)
}
