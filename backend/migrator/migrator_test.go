...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.14.28"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.14/0028##rename_deploy_id_to_replica_id.sql", files[len(files)-1].path)
}
