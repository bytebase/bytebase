...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.14.30"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.14/0030##set_allow_request_role_default.sql", files[len(files)-1].path)
}
