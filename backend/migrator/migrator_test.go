...
func TestLatestVersion(t *testing.T) {
	...
	require.Equal(t, semver.MustParse("3.17.3"), *files[len(files)-1].version)
	require.Equal(t, "migration/3.17/0003##worksheet_composite_pk.sql", files[len(files)-1].path)
}
