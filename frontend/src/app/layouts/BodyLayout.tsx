...
import type { DashboardShellTargets } from "@/app/dashboard-shell";
import {
...
export function BodyLayout() {
  ...
  useEffect(() => {
    const handler = async (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === "A") {
        e.preventDefault();
        const { useAgentStore } = await import("@/modules/agent/store/agent");
  ...
  return (
    <>
      ...
      {targets.quickstart
        ? createPortal(<WorkspaceSetupGuide />, targets.quickstart)
        : null}
    </>
  );
}
