...
import { useTaskDisplay } from "./useTaskDisplay";
import { useTaskStatement } from "./useTaskStatement";
...
const taskRunsForTask = useTaskRunsForTask(
  () => props.task.name,
  () => allTaskRuns.value
);

const latestTaskRun = computed(() => {
  return last(taskRunsForTask.value);
});
