 ...
 * so the table and its consumers cannot drift to an id nothing else knows.
 * Locale coverage is not checked: these keys are built from the id, so a row
 * with no copy renders the raw key.
 */
