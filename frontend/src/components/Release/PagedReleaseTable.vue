<template>
  <slot
    name="table"
    :release-list="state.releaseList"
...
import { useIsLoggedIn, useReleaseStore } from "@/store";
import type { ComposedRelease, Pagination } from "@/types";
...
const sessionState = useSessionStorage<SessionState>(
  `bb.paged-release-data-table.${props.sessionKey}`,
  {
    page: 1,
    updatedTs: 0,
  }
...
const fetchData = (refresh = false) => {
  ...
  request
    .then(({ nextPageToken, releases }) => {
      if (refresh) {
        state.releaseList = releases;
      } else {
        state.releaseList.push(...releases);
      }

      if (releases.length >= expectedRowCount && !isFirstFetch) {
