  ...
  >
    ...
    >
      <div class="flex items-center space-x-1">
        <span
          ...
          :class="[isCreated && !readonly && 'cursor-pointer hover:underline']"
          @click="!readonly && handleClickStageTitle()"
        >
    ...
    >
      <template #default="{ item: task }: { item: Task }">
        <div
          :key="task.name"
          class="w-full border-t border-zinc-50 flex items-center justify-start truncate px-2 py-2 h-10 transition-colors"
          :class="[!readonly && 'cursor-pointer hover:bg-zinc-50']"
          @click="!readonly && handleTaskClick(task)"
        >
