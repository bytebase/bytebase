<template>
  <template v-if="initialized">
    <ArchiveBanner v-if="convertOldStateToNew(project.state) === State.DELETED" class="py-2" />
...
const allowEdit = computed(() => {
  if (convertOldStateToNew(project.value.state) === State.DELETED) {
    return false;
  }
...
const quickActionListForDatabase = computed(() => {
  if (convertOldStateToNew(project.value.state) !== State.ACTIVE) {
    return [];
  }
