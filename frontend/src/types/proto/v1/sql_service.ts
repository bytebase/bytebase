...
export const DifferPreviewRequest: MessageFns<DifferPreviewRequest> = {
  encode(message: DifferPreviewRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.newMetadata !== undefined) {
      DatabaseMetadata.encode(message.newMetadata, writer.uint32(26).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): DifferPreviewRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const DifferPreviewResponse: MessageFns<DifferPreviewResponse> = {
  encode(message: DifferPreviewResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.schema !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): DifferPreviewResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
