...
const contentToDataURL = (content: string, mimeType = "image/svg+xml") => {
  return `data:${mimeType};base64,${btoa(content)}`;
};

const monochromeIconOverrides: MonochromeIconOverride[] = [
  { css: "module", url: contentToDataURL(SchemaIconSVG) },
  { css: "class", url: contentToDataURL(DatabaseIconSVG) },
  { css: "field", url: contentToDataURL(TableIconSVG) },
  { css: "interface", url: contentToDataURL(ColumnIconSVG) },
  { css: "function", url: contentToDataURL(FunctionIconSVG) },
];
...
const coloredIconOverrides: ColoredIconOverride[] = [
  {
    css: "variable",
    url: {
      normal: contentToDataURL(ViewIconSVG.normal),
      active: contentToDataURL(ViewIconSVG.active),
    },
  },
];
...
const STYLE_ID = "bb-react-monaco-suggest-icons";

export const ensureSuggestOverrideStyle = () => {
