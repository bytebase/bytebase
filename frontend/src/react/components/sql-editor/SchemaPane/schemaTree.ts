 ...
 * `src/react/components/sql-editor/ConnectionPane/tree.ts`.
 */
...
export type NodeType =
  ...
  | "check"
  | "dependency-column"
  | "expandable-text" // "Tables / Views / Functions / Triggers" folder headers
  | "error"; // "<Empty>" / "Cannot fetch …" placeholders
 ...
 * recursive `children`.
 */
export type TreeNode<T extends NodeType = NodeType> = {
  key: string;
  meta: NodeMeta<T>;
  children?: TreeNode[];
  label?: string;
...
export const readableTextForNodeTarget = <T extends NodeType>(
  ...
  switch (type) {
    case "error":
      // Empty string keeps error rows out of search matches.
      return "";
...
const mapIndexNodes = (
  ...
  return indexes.map((index) =>
    mapTreeNodeByType("index", { ...target, index: index.name })
  );
};
...
const mapExternalTableNodes = (
  ...
  return schema.externalTables.map((externalTable) => {
...
const mapPartitionTableNodes = (
  ...
  return parentPartition.subpartitions.map((partition) => {
