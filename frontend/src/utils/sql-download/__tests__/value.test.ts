...
describe("csvCellFromStructpbValue (CSV recursive helper)", () => {
  it("renders null", () => {
    expect(
      csvCellFromStructpbValue(
        createProto(ValueSchema, { kind: { case: "nullValue", value: 0 } })
      )
    ).toBe("");
  });
  ...
  it("renders string with surrounding double quotes (no escaping inside, parity quirk)", () => {
    expect(
      csvCellFromStructpbValue(
        createProto(ValueSchema, { kind: { case: "stringValue", value: "ab" } })
      )
    ).toBe(`"ab"`);
  });
  ...
  it("renders number with shortest decimal", () => {
    expect(
      csvCellFromStructpbValue(
        createProto(ValueSchema, { kind: { case: "numberValue", value: 1.5 } })
      )
    ).toBe("1.5");
  });
  ...
  it("renders bool", () => {
    expect(
      csvCellFromStructpbValue(
        createProto(ValueSchema, { kind: { case: "boolValue", value: true } })
      )
    ).toBe("true");
  });

  it("renders list as quoted-bracket JSON-ish", () => {
    const v = createProto(ValueSchema, {
      kind: {
        case: "listValue",
        value: createProto(ListValueSchema, {
          values: [
            createProto(ValueSchema, {
  ...
  it("renders struct with sorted keys (deterministic, divergence from backend)", () => {
    const v = createProto(ValueSchema, {
      kind: {
        case: "structValue",
        value: createProto(StructSchema, {
          fields: {
            b: createProto(ValueSchema, {
              kind: { case: "numberValue", value: 2 },
            }),
            a: createProto(ValueSchema, {
              kind: { case: "numberValue", value: 1 },
            }),
          },
        }),
...
describe("xlsxStringFromStructpbValue (proto .String() shim)", () => {
  it("renders scalar string", () => {
    expect(
      xlsxStringFromStructpbValue(
        createProto(ValueSchema, { kind: { case: "stringValue", value: "hi" } })
      )
    ).toBe(`string_value:"hi"`);
  });

  // B1: prototext escaping for string body. Mirrors Go's proto.String() output.
  it.each([
    [`a"b`, `string_value:"a\\"b"`],
    [`a\\b`, `string_value:"a\\\\b"`],
    ["a\nb", `string_value:"a\\nb"`],
    ["a\rb", `string_value:"a\\rb"`],
    ["a\tb", `string_value:"a\\tb"`],
    ["a\x00b", `string_value:"a\\x00b"`],
    ["a\x1bb", `string_value:"a\\x1bb"`],
    ["a\x07b", `string_value:"a\\x07b"`], // BEL: \x07, NOT \a (differs from goQuoteInner)
    ["a'b", `string_value:"a'b"`], // single quote NOT escaped
    ["中文", `string_value:"中文"`], // printable Unicode passthrough
    ["a\x80b", `string_value:"a\\u0080b"`], // C1 control U+0080 →  (4-digit, matches Go prototext)
    ["a\x9fb", `string_value:"a\\u009fb"`], // C1 control U+009F → 
    ["a\xa0b", `string_value:"a\xa0b"`], // U+00A0 NBSP — printable, not escaped
    ["a­b", `string_value:"a­b"`], // U+00AD soft hyphen — Go emits verbatim
    ["a​b", `string_value:"a​b"`], // U+200B ZWSP — Go emits verbatim
  ])("escapes string body %p as %p", (input, expected) => {
    expect(
      xlsxStringFromStructpbValue(
        createProto(ValueSchema, {
  ...
  it("escapes struct keys with quotes/backslash (B1)", () => {
    const v = createProto(ValueSchema, {
      kind: {
        case: "structValue",
        value: createProto(StructSchema, {
          fields: {
            [`k"y`]: createProto(ValueSchema, {
    ...
    // Note: Go's prototext emits a single space between key and value within
    // a struct field on protobuf-go ≥ v1.36 — see PROTOTEXT_SEP in value.ts.
    expect(xlsxStringFromStructpbValue(v)).toBe(
      `struct_value:{fields:{key:"k\\"y" value:{string_value:"v"}}}`
    );
  });
});
