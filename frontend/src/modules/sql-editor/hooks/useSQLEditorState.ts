...
import { useSQLEditorTabsStore } from "@/modules/sql-editor/store/tab";
...
// Normalize unlimited result-row values before merging workspace and project
// policy limits.
...
/**
 * React access to the active project's QueryDataPolicy. Fetches the
 * workspace-level and project-level DATA_QUERY policies and merges them — the project's
...
export const useSQLEditorQueryDataPolicy = (
  ...
  // policies on its own, so this hook owns the fetch lifecycle.
  useEffect(() => {
 ...
 * project's `maximumResultRows` whenever the policy lowers the cap. Call once
 * from an always-mounted SQL editor shell. The clamp settles
 ...
 * a SQL editor connection. Values update with the underlying Zustand state.
 */
