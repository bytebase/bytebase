  ...
  <div class="w-full overflow-x-hidden flex flex-col gap-y-4 pb-4">
    ...
    >
      <PagedTable
        ...
        :session-key="`bb.paged-service-account-table.project.${projectId}.active`"
    ...
    <div v-if="hasListPermission">
      ...
      <template v-if="state.showInactiveList">
        ...
        <PagedTable
          ...
          :session-key="`bb.paged-service-account-table.project.${projectId}.deleted`"
...
const hasListPermission = computed(() => {
  return hasProjectPermissionV2(project.value, "bb.serviceAccounts.list");
});
