// frontend/scripts/check-layering.mjs
//
// Enforces the semantic overlay layering policy introduced by PR #20012.
...
const main = () => {
  ...
  if (violations.length > 0) {
    console.error(
      `Layering policy violations (${violations.length}). ` +
  ...
  console.log(
    violations.length === 0
      ? "Layering policy: all checks passed."
      : "Layering policy: report-only mode completed with violations."
  );
};
