...
// leave an assertion pointing at a sentence that does not exist. Copy that must
// be ABSENT stays a literal below, since it has no key to read.
...
// The row titles, in list order, derived from the table the product renders, so
// a row added there is covered here without editing this file. The inventory
// itself is pinned in mcpCapabilityRows.test.ts, not here.
