...
**Phase 1: Add `resource_id` column to API-exposed tables**

Add a new `text` column with a generated default, backfill existing rows, then switch the API to use it. This allows a zero-downtime migration.
...
**Phase 2: Switch API layer to use `resource_id`**

Update resource name parsing functions in `backend/common/resource_name.go`:
- `GetProjectIDPlanID()` -- return `string` instead of `int64`
- `GetProjectIDIssueUID()` -- return `string` instead of `int`
- `GetWorksheetUID()` -- return `string` instead of `int`
- etc.

Update store `FindXxxMessage` structs to use `ResourceID *string` instead of `UID *int`.

**Phase 3: Migrate FK references from integer to string**

Tables that reference these IDs via FK need to be updated:

| FK column | References | Action |
|-----------|-----------|--------|
| `task.plan_id` | `plan(id)` | Change to `plan_resource_id text REFERENCES plan(resource_id)` |
| `task_run.task_id` | `task(id)` | Change to `task_resource_id text REFERENCES task(resource_id)` |
| `task_run_log.task_run_id` | `task_run(id)` | Change to `task_run_resource_id text REFERENCES task_run(resource_id)` |
| `issue.plan_id` | `plan(id)` | Change to `plan_resource_id text REFERENCES plan(resource_id)` |
| `issue_comment.issue_id` | `issue(id)` | Change to `issue_resource_id text REFERENCES issue(resource_id)` |
| `worksheet_organizer.worksheet_id` | `worksheet(id)` | Change to `worksheet_resource_id text REFERENCES worksheet(resource_id)` |
| `changelog.sync_history_id` | `sync_history(id)` | Keep as-is (internal, not exposed in API) |
| `plan_check_run.plan_id` | `plan(id)` | Change to `plan_resource_id text REFERENCES plan(resource_id)` |
| `plan_webhook_delivery.plan_id` | `plan(id)` | Change to `plan_resource_id text REFERENCES plan(resource_id)` |

**Phase 4: Drop integer `id` columns from API-exposed tables**

After all FK references are migrated, drop the old integer `id` columns and promote `resource_id` to primary key (same approach as Section 4).

### Code Changes

- **`backend/common/resource_name.go`** -- All `GetXxxID()` functions return `string` instead of `int`/`int64`. Remove `strconv.Atoi()`/`strconv.ParseInt()` calls.
- **Store message types** -- Change `ID int` / `UID int` fields to `ResourceID string`.
- **Store SQL** -- Update all `WHERE id = ?` to `WHERE resource_id = ?`, all `RETURNING id` to `RETURNING resource_id`.
- **Proto** -- No change needed (resource names are already strings in the proto API).
- **Frontend** -- No change needed (the frontend already treats resource IDs as opaque strings in URL paths).
...
## 4. Remove Unused `id` Columns

### Why

Many tables have a `serial`/`bigserial` `id` column as the PRIMARY KEY but never use it for queries -- all lookups, JOINs, and foreign keys use natural keys (`resource_id`, `email`, `name`, composite unique constraints). These unused `id` columns add unnecessary overhead and complicate the schema. Removing them simplifies the data model and makes the natural keys the explicit primary keys.

**Relationship with Section 3:** Section 3 migrates tables that DO use their integer `id` to string `resource_id`. This section removes the `id` column from tables that DON'T use it at all. Both sections can be done independently.

### Tables with Unused `id` Columns

| Table | Natural Key | `id` usage | FK references to `id`? |
|-------|------------|------------|----------------------|
...
| `db` | `(instance, name)` (unique) | None | No (FKs use `(instance, name)`) |
...
| `instance_change_history` | `version` (unique) | No store file (legacy table) | No |

### Tables Where `id` IS Used (Keep or Migrate per Section 3)

These tables actively use `id` in WHERE/JOIN or have FK references from other tables. Tables marked with ★ are covered by Section 3 (migrate integer `id` → string `resource_id`):

| Table | Why `id` is needed | Section 3? |
|-------|-------------------|-----------|
| `plan` | FK from `task`, `issue`, `plan_check_run`, `plan_webhook_delivery` | ★ API-exposed |
| `task` | FK from `task_run` | ★ API-exposed |
| `task_run` | FK from `task_run_log`; used in WHERE for lookups | ★ API-exposed |
| `issue` | FK from `issue_comment`; used in WHERE for lookups | ★ API-exposed |
| `issue_comment` | Used in WHERE/UPDATE by id | ★ API-exposed |
| `revision` | Used in WHERE/DELETE by id | ★ API-exposed |
| `changelog` | Used in WHERE/UPDATE by id | ★ API-exposed |
| `worksheet` | FK from `worksheet_organizer`; used in WHERE/DELETE | ★ API-exposed |
| `project_webhook` | Used in WHERE/UPDATE/DELETE by id | ★ API-exposed |
| `plan_check_run` | Used in WHERE/UPDATE by id | Singleton (no id in API path) |
| `sync_history` | FK from `changelog`; used in WHERE for lookups | Internal only |
| `principal` | Used in WHERE/UPDATE by id | Uses email |
| `service_account` | Used in WHERE/UPDATE by id | Uses email |
| `workload_identity` | Used in WHERE/UPDATE by id | Uses email |
| `audit_log` | Used in ORDER BY, stored in message | Internal only |
| `query_history` | Used in ORDER BY, stored in message | Internal only |
| `export_archive` | Used in WHERE/DELETE by id | Internal only |
| `worksheet_organizer` | Stored in message | Internal only |

### Migration SQL

```sql
-- For each table, drop the id column and promote the natural key to PRIMARY KEY.
-- Order matters: drop tables with no dependents first.
...
-- instance_change_history: natural key = version
ALTER TABLE instance_change_history DROP COLUMN id;
ALTER TABLE instance_change_history ADD PRIMARY KEY (version);
DROP INDEX IF EXISTS idx_instance_change_history_unique_version;
```

### Code Changes

- **Store files**: Remove `id` from SELECT lists, INSERT RETURNING clauses, and message struct fields where applicable.
- **ORDER BY id**: Replace with `ORDER BY created_at` (for `idp`, `task_run_log`, `release`) or `ORDER BY resource_id` where appropriate.
- **release.go**: Remove the `var id int64` scan from the RETURNING clause.
- **Migration test**: Update `TestLatestVersion` in `backend/migrator/migrator_test.go`.
