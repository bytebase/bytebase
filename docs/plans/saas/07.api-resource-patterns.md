...
| QueryHistoryService | SearchQueryHistories (caller-scoped), ListQueryHistories (also accepts wildcard `projects/-` for workspace-wide reads) | parent: `projects/{project}` |
| QueryHistoryService | GetQueryHistory (caller-scoped) | name: `projects/{project}/queryHistories/{queryHistory}` |
...
| DatabaseService | DiffSchema | name: `instances/{instance}/databases/{database}[/changelogs/{changelog}]` |
| DatabaseService | DiffMetadata | name: `instances/{instance}/databases/{database}` |
| DatabaseService | BatchUpdateDatabases, BatchSyncDatabases | parent: `instances/{instance}` |
| ChangelogService | ListChangelogs | parent: `instances/{instance}/databases/{database}` |
| ChangelogService | GetChangelog | name: `instances/{instance}/databases/{database}/changelogs/{changelog}` |
...
- The query history RPCs moved to QueryHistoryService (see the
  `projects/{project}` parent table); SQLService keeps deprecated delegating
  aliases without HTTP bindings until their removal (date TBD)
- DiffMetadata moved to DatabaseService (database-parented, reads the source
  schema from the store, gated by bb.databases.diffMetadata); the old
  anonymous SQLService method and its REST path are removed
- AICompletion is removed; all AI chat goes through AIService.Chat

### AIService — Chat

- Workspace-level utility method (no resource parent)
- CUSTOM auth: authenticated users only, requires the workspace AI setting
  to be enabled
