...
| T13 issue filters | `approval_status` and `current_approver` are matched in SQL before `LIMIT`, so a filtered page can no longer come back empty under a live page token. The status is a `CASE` over `payload->'approval'` mirroring `computeApprovalStatus` branch for branch — including the empty-approvers case that precedes the rejected and approved checks, and reading an absent key as a zero value because payloads are written with a bare `protojson.Marshal`. `current_approver` cannot be resolved in SQL alone (binding conditions and group expansion are evaluated in Go), so the caller resolves which roles the named user holds in each project being searched and passes the `(project, role)` pairs down, the same shape as the saved-query `AccessMembers` predicate; the flow's waiting role is `roles ->> jsonb_array_length(approvers)`, which yields NULL once every step has been acted on. `TestIssueApprovalFiltersRunBeforePaging` locks the paging behavior and `TestIssueApprovalStatusFilterMatchesConverter` pins the SQL derivation to the Go one. Accepted with it: the predicates are not indexable, so a filtered list now scans until it has a full page of matches instead of stopping at the first `LIMIT` rows — the work the Go filter should always have done, but worth an expression index if it shows up in practice. Deliberately unchanged: the filter still ignores rejection and self-approval, so a rejected issue names the holder of the *next* role as its current approver, exactly as before — masked in the default view, which also pins `approval_status == PENDING`, and worth its own fix |
