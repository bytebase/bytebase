...
| T10 | [#21258](https://github.com/bytebase/bytebase/pull/21258) (with [#21251](https://github.com/bytebase/bytebase/pull/21251) and [#21252](https://github.com/bytebase/bytebase/pull/21252)) — password change and every factor-touching method now take a `CredentialProof` (current password, live OTP, recovery code, or a Cloud-only emailed re-auth code) inside a fenced credential transaction that revokes all three token tables and bumps a `cred_gen` claim, so the attacker's own access token dies with the refresh tokens. Design: [`reauthenticate-credential-changes.md`](reauthenticate-credential-changes.md) |
