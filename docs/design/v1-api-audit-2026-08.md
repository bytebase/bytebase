...
| T12 email enumeration, mail relay | Two of the four anonymous surfaces; LDAP disclosure and the existence oracle stay open and are what T12 is now. **`Signup`** checks the restriction before it checks existence, so a workspace that refuses every signup — every SaaS workspace, since the override always sets `DisallowSignup` — no longer answers `AlreadyExists` for registered addresses and `PermissionDenied` for the rest. The duplicate is still reported where signup is allowed; only the order moved, and denied duplicates now reach the deferred `SetAuditWorkspaceID` they used to return ahead of. **`SendEmailLoginCode`** budgets the deployment's own sender at 100/hour — the path a caller reaches by naming no workspace, which carries no admin opt-in and is shared by every tenant, so its reputation is what an anonymous caller with a recipient list can burn. The 60s per-recipient cooldown already bounded one address; only a sender-wide cap bounds a campaign across many, since such a campaign writes to each address exactly once. Budget is claimed before the code row is written, so a refusal leaves a pending code intact, and after a read-only cooldown check, so repeated requests for one address cannot drain it through the cooldown's silent skips. Reuses `login_attempt` under a new `EMAIL_CODE_SEND` kind — no migration, `kind` is text — but not its semantics: a lockout resets only after a quiet window, which for a volume budget turns "100 per hour" into "100 ever, until an hour of silence", so this kind reads `last_attempt_at` as the window's start rather than the latest send. A named workspace is deliberately **not** budgeted: its mail leaves over an SMTP server its own admin configured, so relay abuse and throttling both land on that one customer, and throttling is far cheaper to inflict — the key would be the workspace the *caller* names, and a self-hosted workspace ID is anonymously readable from `GetWorkspace("workspaces/-")`, so a shared bucket would let a few dozen unauthenticated requests turn off sign-in codes for everyone in it. That path's recipient narrowing is the domain restriction and its volume ceiling is the sending provider's own. Nothing here varies by recipient, either: a member exemption and a redeemability gate were both implemented and removed, because on an endpoint that answers anonymous callers any recipient-dependent branch is an account-existence oracle — review found three, each a different spelling of the same mistake |
