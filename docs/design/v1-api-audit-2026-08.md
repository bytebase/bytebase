...
| T18c-i | `BatchDeleteProjects` purges every named project in one transaction, so a failure partway rolls the whole batch back instead of leaving the projects ahead of it irreversibly gone with an error naming only the one that failed. There is one store method where there were two — `DeleteProjects`, variadic like the `UpdateProjects` beside it — and its statements carry `project = ANY(?)`, one per table for the whole batch, in the child-to-parent table order the loop already used, so the chains in [`backend/store/AGENTS.md`](../../backend/store/AGENTS.md#transaction-row-lock-ordering) are walked bottom-up for every project at once. The separate `SELECT ... FOR UPDATE` that guarded the archived state is gone rather than generalized: it only duplicated the `AND deleted = TRUE` on the delete two lines below it, which takes the same row locks at the same point. That delete now carries `RETURNING resource_id`, and a name that does not come back — missing, still active, or in another workspace — fails the batch with the message the single-project purge already used. No ordering is claimed for concurrent purges: the descendant statements that run first never had one either, and [`backend/store/AGENTS.md`](../../backend/store/AGENTS.md#transaction-row-lock-ordering) already treats purge as a point-in-time lifecycle operation whose conflicts are best-effort. Accepted with it: sample-instance cleanup still runs per project *before* the transaction, because it removes an embedded PostgreSQL data directory or an external database and role that no rollback restores — it writes no project row, so a failure there still purges nothing, and `sample_instance_setup` holds one live row per workspace, so at most one project in a batch has work to do. Unchanged: the default-project and archived-before-purge guards are still checked for every name before any purge begins, and the soft-delete path was already a single `UPDATE`. **Shipped with no test of its own, deliberately — the atomicity is unlocked.** Every existing purge test (`TestCollisionDeleteProjectCascade`, the three `project_purge_cache_test.go` cases, `TestDeleteProjectPurgeKeepsOtherProjectSavedQueryStars`, `TestDeleteProjectDeletesProjectInstancesAndKeepsWorkspaceInstanceDatabases`) now drives the batched SQL, so the rewrite of ~25 statements to `project = ANY(?)` is covered — but only ever with one name, so nothing catches a predicate that reaches some projects and not others, and nothing fails if the handler returns to a per-project loop. A handler-level test could not have covered it either: the archived-before-purge guard rejects the whole request before the transaction starts, so a mid-batch failure is unreachable through the API and the invariant is only observable from the store. The batch is also a changed multi-row transaction shipping without the contention coverage step 5 of [`docs/pre-pr-checklist.md`](../../docs/pre-pr-checklist.md) asks for; the sort-before-lock argument is the only thing standing behind the deadlock claim. Judged not worth the test weight. Untouched, and still open as T18c-iv: the batch form is authorized at workspace scope |
