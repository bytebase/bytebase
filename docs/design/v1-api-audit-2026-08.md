...
| T16 | The four `BatchGet` RPCs behaved four different ways when a name resolved to nothing, none of it documented, and the responses are bare repeated lists — so a client matching `names[i]` to `results[i]` silently mis-attributed data. They now answer one contract, stated in each proto and matching AIP-231: **one resource per requested name, in request order, and the first name that does not resolve fails the whole call.** `BatchGetX` is exactly `GetX` per name, so there is nothing new to learn and no partial response to reason about. What that fixed, RPC by RPC: `BatchGetDatabases` dropped a database that was gone or invisible and answered 200 with a short list; `BatchGetGroups` swallowed *every* error into a silent omission, so a store failure was indistinguishable from "no such group"; `BatchGetUsers` emitted the store's `created_at` order rather than request order, which is the mis-attribution hazard in its purest form; `BatchGetProjects` was already atomic but silently dropped a deleted project that `GetProject` returns. An earlier revision of this fix reported unresolved names in an `unmatched_names` field instead of failing — reviewed as overkill, and dropped: partial success is a second contract for every client to handle, and AIP-231 says to use a List method when you want one. Accepted with it, and the reason the partial design was tried first: a caller resolving names held in stored references — IAM bindings, plan and issue targets — now loses the whole batch to one stale name, and on Cloud `BatchGetUsers` scopes to workspace IAM, so an issue created by a since-removed member fails the batch behind that page. Each of the four frontend stores therefore falls back to per-name fetches when the batch fails, which is the pattern `batchFetchProjects` already used for exactly this. `backend/tests/batch_get_test.go` pins request order and the all-or-nothing failure for all four against a real PostgreSQL. Runtime-proven rather than code-traced: all four were driven against a running server |
