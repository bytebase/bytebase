...
| T16 | One contract across all four `BatchGet` RPCs, stated in the proto: resources in request order, a repeated name served once from its first occurrence, and a name that resolves to nothing reported in a new `unmatched_names` field rather than dropped or failing the batch. `BatchGetUsers` was emitting the store's `created_at` order and now reorders in the handler; `BatchGetGroups` swallowed every error into "no such group", and now only `PermissionDenied` joins that bucket while a store failure fails the call — it also stops re-entering the `GetGroup` handler with a synthesized request, which had left the `PermissionDeniedDetail`'s method empty. `unmatched_names` never says which of "gone" or "invisible" applies, so it is not an existence oracle — the same reason `BatchGetDatabases` now counts a database outside the requested `parent` as unmatched instead of `InvalidArgument`, an answer that read the stored row and told a caller authorized on one project that a database lives in another (T18c-iii's shape). **Where the all-or-nothing actually came from**, and the correction to the first reading of it: `getResourceFromRequest` has a `BatchGet` branch that resolves *every* name in `names`, so the interceptor — not the handlers — 404'd the whole batch on one name that did not exist, and under IAM required the permission on *all* named projects, so one project the caller could not see denied the call. Resolution is now lenient for exactly the names a BatchGet looks up (a `NotFound` there skips the name for the handler to report; the batch's `parent` stays strict), and `BatchGetProjects` moved to `CUSTOM` with a per-named-project check, the way `BatchGetDatabases` already worked. The frontend's N+1 `fetchProject` fallback, which had quietly absorbed both failures, is deleted. `BatchGetProjects` also matches `GetProject` on a deleted project now, which it used to drop. Accepted with it: the contract is deliberately not AIP-231-atomic ("must fail for all resources or succeed for all"), because these callers resolve names held in IAM bindings, plans and issues that go stale — and on Cloud `BatchGetUsers` scopes to workspace IAM, so an issue created by a since-removed member would 404 the page. The AIP-conformant alternative, `ListX(filter: 'name in [...]')`, is not reachable: the CEL filter builders take single-value `name == "x"` only. Each RPC's proto comment says so. Also accepted: `findDatabaseForResource` still reports a name-versus-topology mismatch as `InvalidArgument` at its other ten call sites; only its store failure now carries `Internal`, which is what lets `BatchGetDatabases` tell an outage from a bad name. `batch_get_test.go` pins order, dedupe, the unmatched split, abort-on-error and the lenient-name set with no database; `backend/tests/batch_get_test.go` pins all four RPCs against a real PostgreSQL, including the project-scoped role the old resolution denied. Runtime-proven rather than code-traced: all four were driven against a running server, which is what caught the interceptor as the real source |
