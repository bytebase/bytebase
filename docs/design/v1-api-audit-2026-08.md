...
| T12 email enumeration, mail relay | Two of the four anonymous surfaces; LDAP disclosure and the existence oracle stay open and are what T12 is now. **`Signup`** checks the restriction before it checks existence, so a workspace that refuses every signup — every SaaS workspace, since the override always sets `DisallowSignup` — no longer answers `AlreadyExists` for registered addresses and `PermissionDenied` for the rest. The duplicate is still reported where signup is allowed; only the order moved, and denied duplicates now reach the deferred `SetAuditWorkspaceID` they used to return ahead of. **`SendEmailLoginCode`** budgets how much mail one sender may generate: 30/hour for a named workspace, which sends over its own SMTP, and 100/hour for a caller who names none and so reaches the deployment's own `EMAIL_CONFIG` identity, the sender shared by every self-service signup. Both sit far above real volume, and the workspace figure matches the comparable product default (Supabase caps a project at 30 auth OTPs/hour). The 60s per-recipient cooldown already bounded one address; this bounds the campaign across many, which is the finding. Budget is claimed before the code row is written, so a refusal leaves a pending code intact, and after a read-only cooldown check, so repeated requests for one address cannot drain a shared bucket through the cooldown's silent skips. Reuses `login_attempt` under a new `EMAIL_CODE_SEND` kind — no migration, `kind` is text — but not its semantics: a lockout resets only after a quiet window, which for a volume budget turns "30 per hour" into "30 ever, until an hour of silence", so this kind reads `last_attempt_at` as the window's start rather than the latest send. Accepted with it: sustaining either rate delays sign-in codes for that sender until the window passes, the same trade T9's lockout makes. Deliberately **not** varied by recipient — no exemption for members, no gate on whether the address could redeem the code. Both were implemented and removed: this endpoint answers anonymous callers, so any behavior that depends on who the recipient is becomes an account-existence oracle, and review found three separate leaks of exactly that shape before the rule was made structural. A uniform sender-scoped cap costs a busy workspace some throttling and buys back the entire class |
