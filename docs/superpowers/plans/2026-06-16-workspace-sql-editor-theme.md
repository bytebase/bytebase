...
- [ ] **Step 2:** In `SQLEditorSection`, when "Custom" is selected: on first switch, `customDraft = { ...deriveThemeFromAnchors(themeToAnchors(currentResolvedTheme), defaultName), id: crypto.randomUUID() }`; render `<ThemeAnchorEditor>`; on change → `customDraft = { ...deriveThemeFromAnchors(nextAnchors, name), id: keptUuid }` (preserve the uuid across edits). `selectedThemeId = customDraft.id`.
