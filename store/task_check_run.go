...
func (s *TaskCheckRunService) CreateTaskCheckRunIfNeeded(ctx context.Context, create *api.TaskCheckRunCreate) (*api.TaskCheckRun, error) {
	...
	taskCheckRun, err := s.PgCreateTaskCheckRunTx(ctx, tx.Tx, create)
	if err != nil {
		return nil, err
	}
	if _, err := s.CreateTaskCheckRunTx(ctx, tx.PTx, create); err != nil {
		return nil, err
	}
...
func (s *TaskCheckRunService) PatchTaskCheckRunStatus(ctx context.Context, patch *api.TaskCheckRunStatusPatch) (*api.TaskCheckRun, error) {
	...
	taskCheckRun, err := s.PgPatchTaskCheckRunStatusTx(ctx, tx.Tx, patch)
	if err != nil {
		return nil, FormatError(err)
	}
	if _, err := s.PatchTaskCheckRunStatusTx(ctx, tx.PTx, patch); err != nil {
		return nil, FormatError(err)
	}
