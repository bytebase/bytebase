...
func (s *VCSService) CreateVCS(ctx context.Context, create *api.VCSCreate) (*api.VCS, error) {
	...
	vcs, err := pgCreateVCS(ctx, tx.Tx, create)
	if err != nil {
		return nil, err
	}
	if _, err := createVCS(ctx, tx.PTx, create); err != nil {
		return nil, err
	}
...
func (s *VCSService) PatchVCS(ctx context.Context, patch *api.VCSPatch) (*api.VCS, error) {
	...
	vcs, err := pgPatchVCS(ctx, tx.Tx, patch)
	if err != nil {
		return nil, FormatError(err)
	}
	if _, err := patchVCS(ctx, tx.PTx, patch); err != nil {
		return nil, FormatError(err)
	}
...
func (s *VCSService) DeleteVCS(ctx context.Context, delete *api.VCSDelete) error {
	...
	if err := pgDeleteVCS(ctx, tx.Tx, delete); err != nil {
		return FormatError(err)
	}
