...
func (s *ColumnService) CreateColumn(ctx context.Context, create *api.ColumnCreate) (*api.Column, error) {
	...
	column, err := s.pgCreateColumn(ctx, tx.Tx, create)
	if err != nil {
		return nil, err
	}
	if _, err := s.createColumn(ctx, tx.PTx, create); err != nil {
		return nil, err
	}
...
func (s *ColumnService) PatchColumn(ctx context.Context, patch *api.ColumnPatch) (*api.Column, error) {
	...
	column, err := s.pgPatchColumn(ctx, tx.Tx, patch)
	if err != nil {
		return nil, FormatError(err)
	}
	if _, err := s.patchColumn(ctx, tx.PTx, patch); err != nil {
		return nil, FormatError(err)
	}
