...
func (s *ProjectWebhookService) CreateProjectWebhook(ctx context.Context, create *api.ProjectWebhookCreate) (*api.ProjectWebhook, error) {
	...
	projectWebhook, err := pgCreateProjectWebhook(ctx, tx.Tx, create)
	if err != nil {
		return nil, err
	}
	if _, err := createProjectWebhook(ctx, tx.PTx, create); err != nil {
		return nil, err
	}
...
func (s *ProjectWebhookService) PatchProjectWebhook(ctx context.Context, patch *api.ProjectWebhookPatch) (*api.ProjectWebhook, error) {
	...
	projectWebhook, err := pgPatchProjectWebhook(ctx, tx.Tx, patch)
	if err != nil {
		return nil, FormatError(err)
	}
	if _, err := patchProjectWebhook(ctx, tx.PTx, patch); err != nil {
		return nil, FormatError(err)
	}
...
func (s *ProjectWebhookService) DeleteProjectWebhook(ctx context.Context, delete *api.ProjectWebhookDelete) error {
	...
	if err := pgDeleteProjectWebhook(ctx, tx.Tx, delete); err != nil {
		return FormatError(err)
	}
...
func pgCreateProjectWebhook(ctx context.Context, tx *sql.Tx, create *api.ProjectWebhookCreate) (*api.ProjectWebhook, error) {
	// Insert row into database.
	row, err := tx.QueryContext(ctx, `
		INSERT INTO project_webhook (
			creator_id,
...
func pgDeleteProjectWebhook(ctx context.Context, tx *sql.Tx, delete *api.ProjectWebhookDelete) error {
	// Remove row from database.
