...
CREATE TABLE repository (
    ...
    -- Base working directory to save sheet files.
    sheet_base_directory TEXT NOT NULL DEFAULT '',
    -- The file path template for matching the commited migration script.
    file_path_template TEXT NOT NULL DEFAULT '',
    -- The file path template for matching sheet.
...
-- Define sheet enum types.
CREATE TYPE sheet_visibility AS ENUM ('PRIVATE', 'PROJECT', 'PUBLIC');
CREATE TYPE sheet_source AS ENUM ('BYTEBASE', 'VCS');
CREATE TYPE sheet_type AS ENUM ('SQL', 'CUSTOM_SCRIPT');
