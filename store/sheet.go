...
func pgCreateSheet(ctx context.Context, tx *sql.Tx, create *api.SheetCreate) (*api.Sheet, error) {
	row, err := tx.QueryContext(ctx, `
		...
		RETURNING id, creator_id, created_ts, updater_id, updated_ts, instance_id, database_id, project_id, name, statement, visibility
	`,
		create.CreatorID,
		create.CreatorID,
		create.InstanceID,
	...
	if projectID.Valid {
		value := int(projectID.Int32)
		sheet.ProjectID = &value
	}
