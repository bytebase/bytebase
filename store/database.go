...
func (s *DatabaseService) FindDatabase(ctx context.Context, find *api.DatabaseFind) (*api.Database, error) {
	...
	if err != nil {
		...
		return nil, &common.Error{Code: common.ENOTFOUND, Message: fmt.Sprintf("database not found: %+v", find)}
	} else if len(list) > 1 {
		return nil, &common.Error{Code: common.ECONFLICT, Message: fmt.Sprintf("found %d databases with filter %+v, expect 1. ", len(list), find)}
	}
...
func (s *DatabaseService) patchDatabase(ctx context.Context, tx *Tx, patch *api.DatabasePatch) (*api.Database, error) {
	...
	return nil, &common.Error{Code: common.ENOTFOUND, Message: fmt.Sprintf("database ID not found: %d", patch.ID)}
}
