...
func (s *Store) createStageRaw(ctx context.Context, create *api.StageCreate) (*stageRaw, error) {
	...
	stage, err := createStageImpl(ctx, tx.PTx, create)
	if err != nil {
		return nil, err
	}
...
func (s *Store) findStageRaw(ctx context.Context, find *api.StageFind) ([]*stageRaw, error) {
	...
	stageRawList, err := findStageImpl(ctx, tx.PTx, find)
	if err != nil {
		return nil, err
	}
...
func createStageImpl(ctx context.Context, tx *sql.Tx, create *api.StageCreate) (*stageRaw, error) {
	query := `
		INSERT INTO stage (
			creator_id,
			updater_id,
...
func findStageImpl(ctx context.Context, tx *sql.Tx, find *api.StageFind) ([]*stageRaw, error) {
	// Build WHERE clause.
