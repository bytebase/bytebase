...
func (s *StageService) FindStage(ctx context.Context, find *api.StageFind) (*api.Stage, error) {
	...
	if err != nil {
		...
		return nil, &common.Error{Code: common.NotFound, Message: fmt.Sprintf("stage not found: %+v", find)}
	} else if len(list) > 1 {
		return nil, &common.Error{Code: common.Conflict, Message: fmt.Sprintf("found %d stages with filter %+v, expect 1", len(list), find)}
	}
	return list[0], nil
}
