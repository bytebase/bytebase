...
func TestTenant(t *testing.T) {
	...
	err := func() error {
		...
		if len(stagingDatabases) != stagingTenantNumber {
			return fmt.Errorf("invalid number of staging databases %v in project %v, expecting %d database", project.ID, len(databases), stagingTenantNumber)
		}
		if len(prodDatabases) != prodTenantNumber {
			return fmt.Errorf("invalid number of prod databases %v in project %v, expecting %d database", project.ID, len(databases), prodTenantNumber)
		}
		...
		for _, instance := range instances {
			...
			if len(histories) != 2 {
				return fmt.Errorf("invalid number of migration histories, want 2, got %v", len(histories))
			}
			if histories[0].Version == "" {
				return fmt.Errorf("empty migration history(0) version")
			}
			if histories[1].Version == "" {
				return fmt.Errorf("empty migration history(1) version")
			}
			hm1[histories[0].Version] = true
			hm2[histories[1].Version] = true
		}
		if len(hm1) != 1 || len(hm2) != 1 {
			return fmt.Errorf("migration history should have only one version in tenant mode")
		}
		return nil
	}()
	if err != nil {
	...
	err := func() error {
		...
		for _, instance := range instances {
			...
			if histories[0].Version != "ver1" {
				return fmt.Errorf("invalid migration history(0) version, want ver1, got %q", histories[0].Version)
			}
			if histories[1].Version == "" {
				return fmt.Errorf("empty migration history(1) version")
			}
