...
func (driver *Driver) Restore(ctx context.Context, sc *bufio.Scanner, config db.RestoreConfig) (err error) {
	...
	fnExecuteStmt := func(stmt string) error {
		if config.IsGhostTable {
			...
			stmt = rewriteToGhostTableForRestore(stmt, time.Now().Format("20060102150405"))
...
func rewriteToGhostTableForRestore(stmt, timestamp string) string {
	...
	if matchesInsert != nil {
		...
		// matches CONSTRAINT names, only match in one line
		...
		// matches timestamp postfix like chk_xxx_20060102150405
		reTimestampPostfix := regexp.MustCompile("^(.+)" +
		...
		for i, line := range lines {
			if matchesConstraint := reConstraint.FindStringSubmatch(line); matchesConstraint != nil {
				...
				matchesTimestampPostfix := reTimestampPostfix.FindStringSubmatch(constraintName)
				// trim the timestamp postfix if there is one
				if matchesTimestampPostfix != nil {
					constraintName = matchesTimestampPostfix[1]
				}
