...
var (
	...
	// restore SQL rewrite matcher for insert query
	...
	// restore SQL rewrite matcher for create table query
	reCreate = regexp.MustCompile("CREATE TABLE " +
		// Capture group for the table name, leaving back quote ` outside for convenience
		"`([^`]+)` " +
		// Capture group for the rest of the contents
		// Note that there will be multiple lines, so using [\s\S]+ to match them all including line breaks
		`([\s\S]+)`)

	// restore SQL rewrite matcher for CONSTRAINT names, only match in one line
	reConstraint = regexp.MustCompile("(.*)" + // matches anything before constraint name
		"CONSTRAINT `([^`]+)`" + // matches constraint name
		// matches anything after constraint name
		"(.*)")

	// restore SQL rewrite matcher for timestamp suffix, e.g. chk_xxx_20060102150405
	reTimestampSuffix = regexp.MustCompile("^(.+)" +
		`_\d{14}$`) // timestamp suffix
)
...
func (driver *Driver) Restore(ctx context.Context, sc *bufio.Scanner, config db.RestoreConfig) (err error) {
	...
	fnExecuteStmt := func(stmt string) error {
		if config.IsGhostTable {
			driver.l.Debug("original", zap.String("stmt", stmt))
			// restore to ghost tables, need to rewrite SQL
			stmt, err := rewriteToGhostTableForRestore(stmt, config.GhostTableConstraintTimestampSuffix)
			if err != nil {
				return err
			}
			driver.l.Debug("rewritten", zap.String("stmt", stmt))
...
// Ghost table names are generated by appending "_ghost" to the original ones.
// Constraint names are generated by appending a timestamp (to second) to the original ones.
// According to https://dev.mysql.com/doc/refman/8.0/en/identifier-length.html, the max length of a
// constraint name is 64, so we should truncate the original constraint name part to fit in 64 characters.
func rewriteToGhostTableForRestore(stmt, timestamp string) (string, error) {
	...
	if matchesInsert != nil {
		dbPrefix := matchesInsert[1]
		if dbPrefix != "" {
			// Currently we only consider restoring a single database, if db prefix is matched, it is unexpected
			return "", fmt.Errorf("expect dbPrefix to be empty, but get %q", dbPrefix)
		}
		ghostTable := fmt.Sprintf("%s_ghost", matchesInsert[2])
		values := matchesInsert[3]
		return fmt.Sprintf("INSERT INTO %s`%s` VALUES %s", dbPrefix, ghostTable, values), nil
		...
		for i, line := range lines {
			if matchesConstraint := reConstraint.FindStringSubmatch(line); matchesConstraint != nil {
				before := matchesConstraint[1]
				after := matchesConstraint[3]
				constraintName := matchesConstraint[2]

				matchesTimestampSuffix := reTimestampSuffix.FindStringSubmatch(constraintName)
				// trim the timestamp suffix if there is one
				if matchesTimestampSuffix != nil {
					constraintName = matchesTimestampSuffix[1]
				}

				// Append timestamp to the end of the constraint name.
				// We use passed-in timestamp instead of calling time.Now() here to provide injection point for test.
				// According to https://dev.mysql.com/doc/refman/8.0/en/identifier-length.html, the max length of
				// constraint name is 64. We are appending 1 underscore character (_) and 14 digits of timestamp,
				// so we should truncate the original constraint name to <= 49.
				if len(constraintName) >= 49 {
					constraintName = constraintName[:49]
				}
				constraintName = fmt.Sprintf("%s_%s", constraintName, timestamp)

				buf.WriteString(fmt.Sprintf("%sCONSTRAINT `%s`%s", before, constraintName, after))
			} else {
		...
		return fmt.Sprintf("CREATE TABLE `%s` %s", ghostTable, tableDefWithRewrittenConstraintName), nil
	}

	// no rewrite rule maches, return the original statement
	return stmt, nil
}
