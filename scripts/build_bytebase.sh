...
if [ "$(version ${GO_VERSION})" -lt "$(version 1.20)" ];
then
   echo "${RED}Precheck failed.${NC} Require go version >= 1.20. Current version ${GO_VERSION}."; exit 1;
fi
...
# Step 2 - Build the monolithic app by building backend release version together with the backend/server/dist (leveraing embed introduced in Golang 1.20).
