...
# gh: developer convenience, never a boot prerequisite -- WARN, not fatal. Pinned so
# a reboot cannot change the version under an open session. Auth lives in
# /home/*/.config/gh and survives reboots, but not a delete-and-recreate.
GH_VER=2.99.0
[ "$(/usr/local/bin/gh --version 2>/dev/null | awk 'NR==1{print $3}')" = "$GH_VER" ] \
  || { curl -sSL --retry 3 --max-time 180 -o /tmp/gh.tgz \
  ...
  || logger -t devbox-startup "WARN: gh install failed; PR commands unavailable"
